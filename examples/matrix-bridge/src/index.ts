...
export interface KernelBridge {
	...
	queryNodes(
		nodeType: string,
		limit: number,
	): Promise<{ tag: "ok"; val: string[] } | { tag: "err"; val: PluginError }>;
	fetch(
		req: HttpRequest,
	): Promise<{ tag: "ok"; val: HttpResponse } | { tag: "err"; val: PluginError }>;
...
export class MatrixBridgePlugin {
	...
	async ingest(): Promise<number> {
		...
		const rawRooms = JSON.parse(new TextDecoder().decode(roomsResult.val.body)) as {
			chunk: MatrixRoom[];
		};
		...
		for (const room of rawRooms.chunk ?? []) {
			...
			if (result.tag === "ok") stored++;
				...
				this.bridge.log(
					"warn",
					`[matrix-bridge] Failed to store room: ${JSON.stringify(result.val)}`,
				);
			...
			if (room.avatar_url) {
				...
				if (memberResult.tag === "ok") stored++;
					...
					this.bridge.log(
						"warn",
						`[matrix-bridge] Failed to store member: ${JSON.stringify(memberResult.val)}`,
					);
			}
		}
	...
	metadata(): PluginMetadata {
		return {
			name: "Matrix Bridge",
			version: "1.0.0",
			description: "Ingests Matrix room messages and member profiles into the example graph",
