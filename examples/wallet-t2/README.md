...
**trust registry**. The registry is the allow-list of civic issuers a deployment pins — set
`DGK_TRUSTED_ISSUERS` (comma-separated issuer ids) and the CLI threads it into `verify` (or inject
`verifyPolicy` when embedding `buildWalletHost`): a credential whose signature is perfectly valid
but whose issuer is NOT on the list is **rejected** — the anti-fraud point ("emissor não-confiável
recusado mesmo com assinatura válida"). With no registry configured the wallet self-trusts
(offline default), so the rejection is reachable exactly when a real deployment pins its list. Both sides of the exchange ship: `verify-presentation`
