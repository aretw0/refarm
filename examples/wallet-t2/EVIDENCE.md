...
> `sovereignty`, `sovereign` + suas suítes, hoje em `packages/authorization-contract-v1/src/sovereign-signer.test.ts` e `packages/identity-provider-ref/src/identity.test.ts`) mora no bloco **`@refarm.dev/wallet`**;
...
|---|---|---|---|
| **Identidade soberana** (`DGK_SOVEREIGN=1`) | A identidade do cidadão executa como **componente WebAssembly** sob tabela de capacidades _deny-all_; a chave privada **nasce e permanece no sandbox** e é **re-derivada** após perda de dispositivo sem jamais atravessar a fronteira. O processo da carteira **não guarda chave privada**. | **REAL** | `packages/identity-provider-ref/src/identity.test.ts` — "SOVEREIGN: the signer surface exposes no key material": a superfície é exatamente `deriveFromSession`/`publicKey`/`sign`/`verify` e `sign` tem aridade 1, então não existe parâmetro de chave para passar; "SANDBOX": assina sob tabela WASI deny-all, com assinatura ed25519 de 64 bytes verificada. `sovereign.ts` (o provider de identidade WASM soberana). |
| **Jornada de consentimento soberana** (`authorize → present → verify`) | Sob modo soberano, o recibo de autorização é **assinado pela chave WASM soberana** (`ed25519-wasm-sovereign`), não pelo digest de fixture. Um recibo **adulterado falha** na verificação. | **REAL** (sob `DGK_SOVEREIGN=1`) | `packages/identity-provider-ref/src/identity.test.ts` — "SOVEREIGN: the signer surface exposes no key material" (a superfície é exatamente `deriveFromSession`/`publicKey`/`sign`/`verify`, e `sign` tem aridade 1: não há parâmetro de chave para passar) e "SANDBOX: instantiating with the deny-all table still signs" (assina sob tabela WASI deny-all, assinatura ed25519 de 64 bytes verificada). `createSovereignWalletBundle` liga essa identidade ao provedor de credenciais. |
| `wallet share` / `present` (soberano) | A apresentação que a carteira constrói é **assinada dentro do sandbox** (`ed25519-wasm-sovereign` no `proof.signature`). | **REAL** (sob `DGK_SOVEREIGN=1`) | `packages/authorization-contract-v1/src/sovereign-signer.test.ts` — o recibo sai com `proof.algorithm = "ed25519-wasm-sovereign"` e assinatura prefixada pelo mesmo algoritmo; a suíte também recusa um recibo adulterado. |
| Cadeia de consentimento (padrão) | A trilha mínima **autorizar → apresentar → verificar → revogar** funciona em cenário determinístico offline. | **SINTÉTICO** (fixture `fixture-fnv1a`) | `flow.e2e.test.ts` (jornada de consentimento: pedido → prompt → autorizar → apresentar → revogar, e as duas recusas nomeadas); `in-memory.ts` (`createDeterministicSigner`). |
