# Agent guidance — the Lab (Marimo notebook)

> Repo-wide rules of engagement live in the root [`AGENTS.md`](../../../AGENTS.md) (`CLAUDE.md` /
> `GEMINI.md` are symlinks to it — the agnostic convention this file follows too). This file scopes
> guidance to editing the Lab notebook whose source is `analise-grafo.py`; the `.html` + assets it
> exports are DERIVED artifacts (see `.gitignore`) — never commit them, and edit the `.py` source, not the export.

## Marimo notebook assistant

This is guidance for creating and editing data science notebooks using marimo — clear, efficient, and
reproducible analysis with marimo's reactive programming model.
