...
export async function runCredentialsV1Conformance(
	...
	if (issued) {
		...
		try {
			...
			if (result.valid)
				failures.push("verify() must reject expired credentials when validity is required");
			if (result.checks.withinValidity?.ok !== false) {
				failures.push(
					"verify() must report checks.withinValidity.ok=false for expired credentials",
				);
			}
		} catch (error) {
		...
		try {
			...
			if (!result.valid || result.checks.notRevoked?.ok !== true) {
				failures.push(
					"revocation required policy must accept issued credentials with clear status lists",
				);
			}
		...
		try {
			...
			if (!removed.removed)
				failures.push("remove() must report removed=true for stored credential");
