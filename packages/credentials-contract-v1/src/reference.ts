...
function getRevocationStatusRef(credential: VerifiableCredential): CredentialStatusRef | null {
	const refs = Array.isArray(credential.credentialStatus)
		...
		: credential.credentialStatus
			...
			: [];
	return (
		refs.find((ref) => !ref.statusPurpose || ref.statusPurpose === STATUS_PURPOSE_REVOCATION) ??
		null
	);
}
...
export class ReferenceCredentialsProvider implements CredentialsProvider {
	...
	async issue(
		...
		const unsigned: VerifiableCredential = {
			...
			type: statusListed.type.includes(VC_TYPE)
				? statusListed.type
	...
	private async allocateStatusListIndex(statusListId: string): Promise<number> {
		...
		const nextIndex =
			typeof payload.nextIndex === "number" && payload.nextIndex >= 0 ? payload.nextIndex : 0;
		const now = nowIso();
	...
	private async writeStatusList(
		...
		const unsigned: CredentialStatusListCredential = {
			...
			credentialSubject: {
				...
				type: STATUS_LIST_SUBJECT_TYPE,
				statusPurpose:
					status.statusPurpose ?? existingSubject?.statusPurpose ?? STATUS_PURPOSE_REVOCATION,
				encodedList,
			},
		};
	...
	private async verifyCredentialNotRevoked(
		...
		if (!statusList) {
			return fail(
				...
				"credential revocation status could not be resolved",
			);
		}
		...
		try {
			...
			if (!result.valid || result.identity.id !== statusList.issuer) {
				return fail(
					"credential_status_signature_invalid",
					"credential status list signature is invalid",
				);
			}
		} catch (error) {
			return fail(
