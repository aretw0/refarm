import type { Task, TaskConformanceResult, TaskContractAdapter } from "./types.js";
...
export async function runTaskV1Conformance(
	...
	try {
		...
		if (created["@type"] !== "Task") failures.push("create() returned wrong @type");
		if (!created.created_at_ns) failures.push("create() did not set created_at_ns");
		if (!created.updated_at_ns) failures.push("create() did not set updated_at_ns");
	} catch (e) {
	...
	if (created) {
		try {
			...
			else if (fetched["@id"] !== created["@id"]) failures.push("get() returned wrong @id");
		} catch (e) {
	...
	if (created) {
		try {
			...
			if (updated.status !== "active") failures.push("update() did not change status");
	...
	if (created) {
		try {
			...
			if (ev.task_id !== created["@id"]) failures.push("appendEvent() returned wrong task_id");
			if (!ev.timestamp_ns) failures.push("appendEvent() did not set timestamp_ns");
		} catch (e) {
	...
	if (created && adapter.query) {
		...
		try {
			...
			if (!found) failures.push("query({ status: 'active' }) did not return updated task");

			const rootResults = await adapter.query({ parent_task_id: null });
			const rootFound = rootResults.some((t) => t["@id"] === created!["@id"]);
			if (!rootFound) failures.push("query({ parent_task_id: null }) did not return root task");
		} catch (e) {
	...
	if (created && adapter.events) {
		...
		try {
			...
			if (evs.length === 0) failures.push("events() returned empty list after appendEvent()");
	...
	if (adapter.summary) {
		...
		try {
			...
			if (typeof s.total !== "number") failures.push("summary() returned non-numeric total");
	...
	if (created) {
		...
		try {
			...
			if (afterDelete !== null) failures.push("delete() did not remove the task");
		} catch (e) {
