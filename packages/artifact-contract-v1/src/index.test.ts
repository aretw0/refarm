...
describe("TaskArtifactManifest", () => {
	...
	it("selects task artifacts by consumer-facing metadata", () => {
		...
		expect(
			selectTaskArtifacts(manifest, { roles: ["audit-trail"] }).map((item) => item.id),
		).toEqual(["wallet-audit-trail"]);
		expect(
			selectTaskArtifacts(manifest, { reviewStates: ["unreviewed"] }).map((item) => item.id),
		).toEqual(["wallet-dataset"]);
		expect(
			selectTaskArtifacts(manifest, { labels: ["vault", "reviewed"] }).map((item) => item.id),
		).toEqual(["wallet-audit-trail"]);
		expect(
			selectTaskArtifacts(manifest, {
	...
	it("validates vault-seed Lab/outbox/notebook evidence without owning notebook UX", () => {
		...
		expect(
			selectTaskArtifacts(manifest, { roles: ["manifest"], labels: ["outbox"] }).map(
				(item) => item.id,
			),
		).toEqual(["publication-outbox"]);
	});
