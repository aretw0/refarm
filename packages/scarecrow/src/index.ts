...
export type ScarecrowHost = RuntimeObserverTarget &
	RuntimePluginStateTarget &
	RuntimeQueryTarget &
	RuntimeTelemetryTarget;
...
export class ScarecrowPlugin {
	...
	private setupObserver() {
		this.host.observe((data: RuntimeTelemetryEvent) => {
			...
			if (
				data.event === "ui:performance" &&
				updateVelocity !== undefined &&
				updateVelocity > this._config.maxUpdateVelocity
			) {
				const pluginId = data.pluginId || "unknown";
				this.emitAlert(
					pluginId,
					`Excessive DOM updates (${updateVelocity}/sec, threshold: ${this._config.maxUpdateVelocity})`,
				);
			...
			if (
				data.event === "ui:a11y_audit" &&
				a11yScore !== undefined &&
				a11yScore < this._config.minA11yScore
			) {
				this.emitAlert(
					data.pluginId || "unknown",
					`Low Accessibility Score (${a11yScore}, threshold: ${this._config.minA11yScore})`,
				);
			}
