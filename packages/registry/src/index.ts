...
export class Registry {
	...
	private async _loadState(): Promise<void> {
		...
		try {
			...
			if ((e as NodeJS.ErrnoException)?.code !== "ENOENT") {
				console.warn(
					`[registry] Could not load persisted state from ${this._persistPath}:`,
	...
	async resolveRemote(id: string, sourceUrl: string): Promise<RegistryEntry> {
		...
		try {
			...
			throw new Error(
				`Remote resolution failed for ${id}: ${e instanceof Error ? e.message : String(e)}`,
			);
		}
	}
	...
	async activatePlugin(id: string): Promise<void> {
		...
		if (plugin.status !== "validated") {
			throw new Error(
				`Plugin ${id} must be validated before activation (current status: ${plugin.status})`,
			);
		}
