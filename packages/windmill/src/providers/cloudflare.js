...
export class CloudflareProvider {
	...
	async syncRecords(desiredRecords, options = {}) {
		...
		for (const desired of desiredRecords) {
			const existing = actualRecords.find(
				(r) => r.name === desired.name && r.type === desired.type,
			);
		...
		if (options.dryRun) {
			...
			for (const change of changes) {
				console.log(
					`  - ${change.action.toUpperCase()}: ${change.record.name} (${change.record.type}) -> ${change.record.content}`,
				);
			}
	...
	async deployPages(siteName, projectDir, options = {}) {
		...
		if (options.dryRun) {
			console.log(
				`[Cloudflare] [DRY RUN] Would deploy artifacts from ${projectDir} to ${siteName}.`,
			);
