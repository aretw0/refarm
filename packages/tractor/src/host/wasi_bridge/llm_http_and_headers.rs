fn sanitized_plugin_headers(headers: &[(String, String)]) -> Vec<(&str, &str)> {
    ...
    for (name, value) in headers.iter().take(MAX_HEADER_SCAN) {
        ...
        if n.is_empty()
            ...
            || n == "x-envoyoriginalpath"
            || n == "x-envoy-original-url"
            || n == "x-envoyoriginalurl"
            || n == "x-client-ip"
            || n == "x-clientip"
            || n == "true-client-ip"
            || n == "trueclientip"
