...
fn is_blocked_spawn_env_key(key: &str) -> bool {
    ...
    if upper.starts_with("LD_")
        ...
        || upper.ends_with("_CFACCESSJWTASSERTION")
        || upper.ends_with("_GOOGIAPJWTASSERTION")
        ...
        || upper.ends_with("_AWSEC2METADATATOKEN")
        || upper.ends_with("_AWSEC2METADATATOKENTTLSECONDS")
        || upper.ends_with("_AWSCONTAINERCREDENTIALSRELATIVEURI")
        || upper.ends_with("_AWSCONTAINERCREDENTIALSFULLURI")
        || upper.ends_with("_AWSCONTAINERAUTHORIZATIONTOKEN")
        || upper.ends_with("_AWSWEBIDENTITYTOKENFILE")
        || upper.ends_with("_METADATAFLAVOR")
        || upper.ends_with("_GOOGLEMETADATAREQUEST")
        || upper.ends_with("_GOOGLEAPPLICATIONCREDENTIALS")
        || upper.ends_with("_GCEMETADATAHOST")
        || upper.ends_with("_CLOUDSDKAUTHCREDENTIALFILEOVERRIDE")
        || upper.ends_with("_AZUREFEDERATEDTOKENFILE")
        || upper.ends_with("_IDENTITYENDPOINT")
        || upper.ends_with("_IDENTITYHEADER")
        || upper.ends_with("_IMDSENDPOINT")
        || upper.ends_with("_MSIENDPOINT")
        || upper.ends_with("_KUBECONFIG")
        || upper.ends_with("_KUBECONFIGPATH")
        ...
        || upper.ends_with("_K8SAWSID")
        || upper.ends_with("_DOCKER_HOST")
        || upper.ends_with("_DOCKERHOST")
        || upper.ends_with("_REGISTRY_URL")
        || upper.ends_with("_REGISTRYURL")
        || upper.ends_with("_CONTAINERS_REGISTRIES_CONF")
        || upper.ends_with("_CONTAINERSREGISTRIESCONF")
        || upper.ends_with("_GHCR_HOST")
        || upper.ends_with("_GHCRHOST")
        || upper.ends_with("_QUAY_ORGANIZATION")
        || upper.ends_with("_QUAYORGANIZATION")
        || upper.ends_with("_HARBOR_URL")
        || upper.ends_with("_HARBORURL")
        || upper.ends_with("_ARTIFACTORY_URL")
        || upper.ends_with("_ARTIFACTORYURL")
        || upper.ends_with("_JFROG_URL")
        || upper.ends_with("_JFROGURL")
        ...
        || upper.ends_with("_CFACCESSCLIENTID")
        || upper.ends_with("_CFACCESSCLIENTSECRET")
        || upper.ends_with("_CLOUDFLAREACCESSCLIENTID")
        || upper.ends_with("_CLOUDFLAREACCESSCLIENTSECRET")
        || upper.ends_with("_GITHUBTOKEN")
        || upper.ends_with("_GITHUBPAT")
        || upper.ends_with("_GITHUBUSERID")
        || upper.ends_with("_GITHUBUSEREMAIL")
        || upper.ends_with("_GITHUBLOGIN")
        || upper.ends_with("_GITLABTOKEN")
        || upper.ends_with("_GITLABPRIVATETOKEN")
        || upper.ends_with("_GITLABCITOKEN")
        || upper.ends_with("_GITLABWEBHOOKSECRETTOKEN")
        || upper.ends_with("_GITLABUSERID")
        || upper.ends_with("_GITLABUSERNAME")
        || upper.ends_with("_GITLABUSERLOGIN")
        || upper.ends_with("_GITLABUSEREMAIL")
        || upper.ends_with("_BITBUCKETTOKEN")
        || upper.ends_with("_BITBUCKETAPPPASSWORD")
        || upper.ends_with("_BITBUCKETUSER")
        || upper.ends_with("_BITBUCKETUUID")
        || upper.ends_with("_BITBUCKETUSEREMAIL")
        || upper.ends_with("_SLACKBOTTOKEN")
        || upper.ends_with("_SLACKAPPTOKEN")
        || upper.ends_with("_SLACKSIGNINGSECRET")
        || upper.ends_with("_SLACKSIGNATURE")
        || upper.ends_with("_SLACKREQUESTTIMESTAMP")
        || upper.ends_with("_SLACKWEBHOOKURL")
        || upper.ends_with("_DISCORDTOKEN")
        || upper.ends_with("_DISCORDWEBHOOKURL")
        || upper.ends_with("_STRIPEAPIKEY")
        || upper.ends_with("_STRIPESECRETKEY")
        || upper.ends_with("_STRIPESIGNATURE")
        || upper.ends_with("_STRIPEWEBHOOKSECRET")
