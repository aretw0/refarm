...
function createBenchManifest(id) {
    return {
        id,
        name: `Manifest ${id}`,
        version: "0.1.0",
...
describe("Plugin Loading", () => {
    ...
    bench("Load 50 plugins concurrently", async () => {
        const host = new PluginHost();
        await Promise.all(Array.from({ length: 50 }, (_, i) => host.load(createBenchManifest(`p${i}`), `h${i}`)));
        host.terminateAll();
    });
    bench("Load 100 plugins concurrently", async () => {
        const host = new PluginHost();
        await Promise.all(Array.from({ length: 100 }, (_, i) => host.load(createBenchManifest(`p${i}`), `h${i}`)));
        host.terminateAll();
    });
});
// ─── Storage Throughput ───────────────────────────────────────────────────────
describe("Storage Throughput", () => {
    bench("storeNode() x1", async () => {
        ...
        const node = normaliseToSovereignGraph({ "@id": "urn:bench:1", name: "Bench" }, "bench-plugin", "BenchNode");
    ...
    bench("storeNode() x100 sequential", async () => {
        ...
        const tractor = await Tractor.boot(config);
        for (let i = 0; i < 100; i++) {
            const node = normaliseToSovereignGraph({ "@id": `urn:bench:${i}`, name: `N${i}` }, "bench-plugin", "BenchNode");
            await tractor.storeNode(node);
        }
        await tractor.shutdown();
    });
...
describe("Full Lifecycle", () => {
    ...
    bench("Boot → Load 10 plugins → Store 50 nodes → Query → Shutdown", async () => {
        ...
        // Store nodes
        for (let i = 0; i < 50; i++) {
            const node = normaliseToSovereignGraph({ "@id": `urn:lc:${i}`, name: `LC ${i}` }, "lifecycle-plugin", "LifecycleNode");
            await tractor.storeNode(node);
        }
        // Query
...
//# sourceMappingURL=stress.bench.js.map