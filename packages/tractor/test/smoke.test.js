...
describe("@refarm/tractor smoke", () => {
    ...
    it("normalises plugin payload into sovereign node", () => {
        const node = normaliseToSovereignGraph({ "@id": "urn:test:1", name: "hello" }, "plugin-smoke", "Note");
    ...
    it("loads plugin handle and tracks instance lifecycle", async () => {
        ...
        const instance = await host.load("https://example.test/plugin.wasm", "hash-placeholder", "plugin-1");
