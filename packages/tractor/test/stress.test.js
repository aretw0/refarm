...
import { MockIdentityAdapter, MockStorageAdapter, createMockConfig } from "./helpers/mock-adapters";
// ─── Shared Mock Manifest Factory ─────────────────────────────────────────────
function createMockManifest(id) {
    return {
        id,
        name: `Manifest ${id}`,
        version: "0.1.0",
...
describe("Boot Stress", () => {
    ...
    it("boots 50 Tractor instances concurrently", async () => {
        const boots = Array.from({ length: 50 }, () => Tractor.boot(createMockConfig()));
    ...
    it("rejects boot without storage adapter", async () => {
        await expect(Tractor.boot({ storage: null, identity: new MockIdentityAdapter() })).rejects.toThrow("[tractor] A Storage Adapter is required to boot.");
    });
    it("rejects boot without identity adapter", async () => {
        await expect(Tractor.boot({ storage: new MockStorageAdapter(), identity: null })).rejects.toThrow("[tractor] An Identity Adapter is required to boot.");
    });
});
// ─── Plugin Flood ─────────────────────────────────────────────────────────────
describe("Plugin Flood", () => {
    ...
    it("loads 100 plugins concurrently", async () => {
        stubFetch();
        const host = new PluginHost();
        const loads = Array.from({ length: 100 }, (_, i) => host.load(createMockManifest(`plugin-${i}`), `hash-${i}`));
    ...
    it("loads 500 plugins concurrently within 2 seconds", async () => {
        ...
        const host = new PluginHost();
        const start = performance.now();
        const loads = Array.from({ length: 500 }, (_, i) => host.load(createMockManifest(`plugin-${i}`), `hash-${i}`));
        await Promise.all(loads);
        const elapsed = performance.now() - start;
    ...
    it("handles plugin load failure gracefully", async () => {
        ...
        const host = new PluginHost();
        await expect(host.load(createMockManifest("broken-plugin"), "hash")).rejects.toThrow("[tractor] Failed to fetch plugin: Not Found");
    });
    it("plugin IDs are unique — no collisions under flood", async () => {
        stubFetch();
        const host = new PluginHost();
        const ids = Array.from({ length: 200 }, (_, i) => `plugin-${i}`);
        await Promise.all(ids.map((id) => host.load(createMockManifest(id), `hash-${id}`)));
...
describe("Storage Throughput", () => {
    it("stores 1000 nodes sequentially", async () => {
        ...
        for (let i = 0; i < 1000; i++) {
            const node = normaliseToSovereignGraph({ "@id": `urn:test:node-${i}`, name: `Node ${i}` }, "stress-plugin", "TestNode");
...
describe("Normaliser Throughput", () => {
    it("normalises 10,000 payloads without UUID collision", () => {
        ...
        for (let i = 0; i < 10_000; i++) {
            const node = normaliseToSovereignGraph({ name: `Item ${i}` }, // no @id → forces UUID generation
            "bench-plugin", "BenchNode");
            ids.add(node["@id"]);
        }
    ...
    it("preserves @id when supplied", () => {
        const node = normaliseToSovereignGraph({ "@id": "urn:explicit:1", name: "test" }, "p1", "T1");
