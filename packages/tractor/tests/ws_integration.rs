...
async fn ws_server_readiness_contract_initial_binary_frame_arrives_quickly() {
    ...
    assert!(
        ...
        "readiness initial frame must be binary"
    );
}
...
async fn ws_server_sends_state_on_connect() {
    ...
    sync.store_node("urn:test:pre-1", "Note", None, "{}", None)
        .unwrap();
    let port = start_server(sync).await;
    ...
    if let Message::Binary(bytes) = msg {
        ...
        doc.import(&bytes)
            .expect("server state must be valid Loro bytes");
    }
}
...
async fn ws_server_applies_incoming_update() {
    ...
    client_sync
        .store_node("urn:test:from-client", "Task", None, "{}", None)
        .unwrap();
    ...
    assert!(
        node.is_some(),
        "server must have node after receiving client update"
    );
}
...
async fn ws_server_broadcasts_to_other_clients() {
    ...
    sender_sync
        .store_node("urn:test:broadcast-1", "Note", None, "{}", None)
        .unwrap();
    ...
    let msg = tokio::time::timeout(std::time::Duration::from_millis(500), ws_b.next())
        .await
...
async fn ws_server_on_update_broadcasts_local_changes() {
    ...
    sync.store_node("urn:test:local-1", "Event", None, "{}", None)
        .unwrap();

    // Connected client must receive the delta via on_update broadcast
    let msg = tokio::time::timeout(std::time::Duration::from_millis(500), ws.next())
        .await
        .expect("timeout waiting for local update broadcast")
        .unwrap()
        .unwrap();

    assert!(
        matches!(msg, Message::Binary(_)),
        "local update must be broadcast as binary"
    );
}
...
async fn ws_server_run_twice_no_duplicate_broadcasts() {
    ...
    sync.store_node("urn:test:dedup-1", "Event", None, "{}", None)
        .unwrap();
    ...
    assert_eq!(
        frames.len(),
        1,
        "exactly 1 broadcast expected — got {} (duplicate callbacks accumulating?)",
        frames.len()
    );
}
...
async fn ws_server_corrupted_bytes_not_relayed() {
    ...
    ws_a.send(Message::Binary(b"not-valid-loro-bytes".to_vec()))
        .await
        .unwrap();
    ...
    assert!(
        result.is_err(),
        "client B must not receive a corrupted frame"
    );
}
...
async fn ws_server_sender_does_not_receive_own_relay() {
    ...
    sender_sync
        .store_node("urn:test:echo-1", "Note", None, "{}", None)
        .unwrap();
    let bytes = sender_sync.get_update().unwrap();
    ws_a.send(Message::Binary(bytes)).await.unwrap();
    ...
    let msg = tokio::time::timeout(std::time::Duration::from_millis(500), ws_b.next())
        .await
        .expect("B must receive relay")
        .unwrap()
        .unwrap();
    assert!(
        matches!(msg, Message::Binary(_)),
        "relay to B must be binary"
    );

    // Client A must NOT receive its own frame back
    let echo = tokio::time::timeout(std::time::Duration::from_millis(200), ws_a.next()).await;
...
async fn ws_server_client_disconnect_no_zombie() {
    ...
    sync.store_node("urn:test:zombie-check", "Note", None, "{}", None)
        .unwrap();

    let msg = tokio::time::timeout(std::time::Duration::from_millis(500), ws.next())
        .await
        .expect("legitimate client must receive broadcast")
        .unwrap()
        .unwrap();

    assert!(
        matches!(msg, Message::Binary(_)),
        "broadcast must be binary — no zombie panic"
    );
}
