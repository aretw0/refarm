...
| `@refarm.dev/source-web` | implemented; not selected | authenticated web capture fixture adapter passes `source:v1` conformance as a redacted local snapshot with session/cache/pacing/offline replay provenance; promotion waits for downstream proof with real login/selectors kept outside Refarm |
...
| `@refarm.dev/enrichment-contract-v1` | implemented; not selected | deterministic record/note enrichment now has a neutral fixture provider and conformance suite; promotion waits for downstream proof with private providers kept outside Refarm |
| `@refarm.dev/records-contract-v1` | implemented; not selected | requirement-like records now have a neutral manifest contract with reference validation, relation integrity, stable hashes, and forward-safe upcast; promotion waits for a downstream proof and composition evidence with source/artifact packages |
