...
| `@refarm.dev/skill-contract-v1` | implemented; not selected | native `skill:v1` manifest/plan/request/decision/surface helpers are checked, plugin-manifest validates `pi/skill` package surfaces, and a plan-only Refarm git-workflow surface smoke records a host policy decision without execution; publication waits for an engine dogfood smoke that invokes one skill without bypassing policy |
