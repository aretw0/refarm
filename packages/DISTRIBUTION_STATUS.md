...
| `@refarm.dev/skill-contract-v1` | implemented; not selected | native `skill:v1` manifest contract is checked, but publication waits for a plugin-manifest skill surface plus Refarm wrapper/adapter smoke that invokes one external skill without bypassing policy |
