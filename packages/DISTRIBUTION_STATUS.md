...
| `@refarm.dev/skill-contract-v1` | implemented; not selected | native `skill:v1` manifest/plan/request/surface helpers are checked, but publication waits for central plugin-manifest skill validation plus Refarm wrapper/adapter smoke that invokes one external skill without bypassing policy |
