...
| `@refarm.dev/skill-contract-v1` | implemented; not selected | native `skill:v1` manifest/plan/request/decision/receipt/surface helpers are checked, plugin-manifest validates `pi/skill` package surfaces, the plan-only Refarm git-workflow smoke records a host policy decision, and the source-status smoke records one `source:v1` engine call through `@refarm.dev/source-local`; publication waits for one external `dgk-skills` or `agents-lab` fixture proof without bypassing policy |
