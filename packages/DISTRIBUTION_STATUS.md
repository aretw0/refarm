...
| `@refarm.dev/skill-contract-v1` | implemented; not selected | native `skill:v1` manifest/plan/request/decision/receipt/surface helpers are checked, plugin-manifest validates `pi/skill` package surfaces, the plan-only Refarm git-workflow smoke records a host policy decision, the source-status smoke records one `source:v1` engine call through `@refarm.dev/source-local`, and the `agents-lab` git-workflow wrapper smoke records external source evidence without installing upstream skill text; publication waits for one `dgk-skills` fixture proof without bypassing policy |
