...
/**
 * The DATA of the capability index — the large declarative literals (the
 * CAPABILITIES catalog + the reference-driver supply/adoption tables) extracted
 * verbatim from capability-index.ts to keep that file to its type contracts +
 * builders. This is a pure data leaf: the four builders in capability-index.ts
 * read these consts; nothing reads back except types (import type above, erased,
 * so no runtime cycle). Each const is `export`ed inline for the builders.
 */

export const REFERENCE_DRIVER_PUBLICATION_BOUNDARY: ReferenceDriverPublicationBoundary = {
	...
	return [
		{
			...
			requirements: ["Runtime ready", "Usable model route", "runtime-agent plugin loaded"],
			policy: {
				state: "proven",
				enforcement: [
		...
		},
		{
			...
			policy: {
				state: "proven",
				enforcement: ["schema validation", "freshness warning", "explicit write command"],
				evidence: [
		...
		},
		{
			...
			description:
				"Validate, list, write, lifecycle, and trigger project-local automations that resume can surface without running a daemon.",
			provider: {
				kind: "sdk",
				package: "@refarm.dev/cli",
		...
		},
		{
			...
			description: "Run scoped end-of-slice validation with machine-readable next commands.",
			provider: {
				kind: "cli",
				package: "@refarm.dev/cli",
		...
		},
		{
			...
			requirements: ["MODEL_SHELL_ALLOWLIST", "MODEL_FS_ROOT", "trusted_plugins"],
			policy: {
				state: "proven",
				enforcement: [
		...
		},
		{
			...
			requirements: ["Tractor observation stream", "StreamSession nodes", "StreamChunk nodes"],
			policy: {
				state: "proven",
				enforcement: ["schema-neutral BrowserSyncClient", "surface capability filtering"],
				evidence: [
		...
		},
		{
			...
			requirements: ["context packet", "allowed toolset", "model route", "bounded budget"],
			policy: {
				state: "governed",
				enforcement: [
		...
		},
		{
			...
			requirements: ["CRDT Session nodes", "SessionEntry tree", "runtime-agent tool dispatch"],
			policy: {
				state: "proven",
				enforcement: [
		...
		},
		{
			...
			requirements: ["host-effects structured-io", "filesystem capability", "format parser"],
			policy: {
				state: "proven",
				enforcement: ["format validation before write", "paged reads", "atomic write path"],
				evidence: [
		...
		},
		{
			...
			requirements: ["connected language server", "host code-ops bridge", "source checkout"],
			policy: {
				state: "governed",
				enforcement: [
		...
		},
		{
			...
			requirements: ["local job store", "ownership metadata", "resume visibility"],
			policy: {
				state: "governed",
				enforcement: ["durable owner", "fail-closed model route", "health/resume visibility"],
				evidence: [
...
export const REFERENCE_DRIVER_SUPPLY_TARGETS = {
		...
		targets: [
			{
				...
				status: "candidate",
				note: "Operator CLI ask loop is source-proven and mock-runtime proven; broader gateway/RPC promotion waits for steering, abort, resume, and event contract proofs.",
			},
			{
				channel: "npm",
				...
				status: "exported",
				note: "Product-neutral local-loop descriptor and gateway/RPC readiness SDK for consumers that need ask-loop promotion status without importing the app.",
			},
			{
				channel: "npm",
				...
				path: "packages/agent",
				status: "hold",
				note: "Runtime-agent package remains private until plugin artifact policy and daily-driver mileage justify publishing the interaction engine.",
			},
		],
		nextDecision:
		...
		targets: [
			{
				...
				status: "exported",
				note: "Plan-only worker descriptor and readiness SDK for consumers that want agents-as-tools without runtime fanout; also available through @refarm.dev/cli/worker-profile.",
			},
			{
				channel: "npm",
				...
				status: "exported",
				note: "Dedicated worker profile subpath for consumers that only need bounded delegated-worker descriptors, readiness blockers, provider token-use budgets, stop conditions, and result envelopes.",
			},
			{
				channel: "npm",
				...
				path: "packages/cli/src/worker-profile.ts",
				status: "exported",
				note: "Worker result envelope requires compact summaries, declared output fields, handoffs, and issues for non-completed statuses; also available through @refarm.dev/cli/worker-profile.",
			},
			{
				channel: "runtime",
				...
				status: "candidate",
				note: "assessWorkerToolReadiness() is the promotion gate; runtime-dispatch remains blocked until local policy, cancellation, observability, provider budget, and cost-control proofs exist.",
			},
			{
				channel: "npm",
				name: "@refarm.dev/agent",
				path: "packages/agent",
				status: "hold",
				note: "Runtime worker execution still belongs behind the private plugin boundary until dispatch policy is proven.",
			},
		],
		nextDecision:
		...
		targets: [
			{
				...
				status: "exported",
				note: "Discovery SDK for the reference-driver capability surface; it does not dispatch runtime tools.",
			},
			{
				channel: "npm",
				name: "@refarm.dev/agent",
				path: "packages/agent",
				status: "hold",
				note: "Runtime plugin package is still private; publish only after plugin artifact policy and daily-driver gate.",
			},
			{
				channel: "runtime",
				...
				status: "candidate",
				note: "Operator command exercises the session-tree primitive without exposing a separate SDK yet.",
			},
		],
		nextDecision:
	...
	"runtime-agent.structured-io": {
		targets: [
			...
			},
			{
				...
				path: "packages/agent",
				status: "hold",
				note: "Runtime plugin consumes structured tools, but its npm package remains private until plugin distribution is ready.",
			},
		],
		nextDecision:
	...
	"runtime-agent.code-ops": {
		targets: [
			{
				channel: "wit",
				name: "plugin:host@0.1.0",
				path: "packages/plugin-wit/wit/worlds.wit",
				status: "internal",
			...
			},
			{
				...
				status: "hold",
				note: "Host LSP bridge implementation remains a reference runtime until the daily-driver gate allows publishing runtime crates.",
			},
		],
		nextDecision:
...
export const REFERENCE_DRIVER_LESSONS: Record<string, readonly string[]> = {
	"runtime-agent.ask": [
...
export const REFERENCE_DRIVER_ADOPTION_CRITERIA = [
	{
	...
	},
	{
		...
		proof: "Gateway promotion stays blocked until steering and abort/resume proofs are executable.",
		consumerBoundary:
...
export const REFERENCE_DRIVER_SOURCE_REFERENCES: Record<
	string,
...
export const REFERENCE_DRIVER_PROMOTION_PROOF_TARGETS: Record<string, readonly string[]> = {
	"runtime-agent.ask": INTERACTION_DRIVER_GATEWAY_BLOCKERS.map((blocker) => blocker.proofTarget),
