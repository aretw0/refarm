...
describe("environment pressure", () => {
	...
	it("classifies oversized session files without scanning the workspace", () => {
		...
		expect(budget.oversized.map((file) => file.level)).toEqual(["warning", "failure"]);
	...
	it("treats huge session pressure as advisory unless resume is requested", () => {
		const baseOptions = {
			...
			sessionFiles: [{ path: ".sessions/huge.jsonl", bytes: 160 * MiB }],
		};
		...
		expect(newSessionReport).toEqual(
			expect.objectContaining({
				ok: true,
				decision: "safe-mode",
			}),
		);
		expect(newSessionReport.signals).toEqual(
			expect.arrayContaining([
				expect.objectContaining({
		...
		expect(resumeReport).toEqual(
			expect.objectContaining({
				ok: false,
				decision: "stop-and-investigate",
			}),
		);
		expect(resumeReport.signals).toEqual(
			expect.arrayContaining([
				expect.objectContaining({
