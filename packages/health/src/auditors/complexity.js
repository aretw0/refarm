	...
	constructor(options = {}) {
		this.#maxLines =
			Number.isFinite(options.maxLines) && options.maxLines > 0
				...
				: DEFAULT_MAX_LINES;
		this.#paths = Array.isArray(options.paths) && options.paths.length > 0 ? options.paths : ["."];
		...
		this.#allowedRules = Array.isArray(options.allowedRules)
			...
			: [];
		this.#reportLimit =
			Number.isFinite(options.reportLimit) && options.reportLimit > 0
