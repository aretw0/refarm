...
export class ProjectAuditor {
	...
	isNonTsPackage(pkgPath) {
		...
		if (
			...
			!fs.existsSync(path.join(pkgPath, "tsconfig.json"))
		)
			return true;
		...
		try {
			...
			return [
				projectAutomationIssue(
					relativePath,
					"invalid_project_automations_json",
					"Project automations must be valid JSON.",
				),
			];
		}
...
function validateProjectAutomationRecord(record, file, index) {
	...
	record.triggers.forEach((trigger, triggerIndex) => {
		issues.push(
			...validateProjectAutomationTrigger(trigger, file, `${suffix}.triggers[${triggerIndex}]`),
		);
	});
	return issues;
}
...
function validateProjectAutomationTrigger(trigger, file, suffix) {
	...
	if (
		trigger.type === "event" &&
		(typeof trigger.eventType !== "string" || trigger.eventType.trim().length === 0)
	) {
		return [
			projectAutomationIssue(
				file,
