...
function getAllFiles(dirPath, arrayOfFiles) {
	...
	files.forEach(function (file) {
		if (fs.statSync(dirPath + "/" + file).isDirectory()) {
			...
			if (
				[".md", ".ts", ".mjs", ".json", ".jsonld", ".yml", ".yaml", ".wit"].includes(ext) ||
				file === "package.json"
			) {
...
async function main() {
	...
	console.log(
		"This script will perform a highly destructive, atomic find-and-replace across the entire monorepo.",
	);
	...
	try {
		...
		console.error(
			`${installCommand.display} failed, you may need to resolve some manual package name conflicts in package.json`,
		);
	}
