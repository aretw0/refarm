import { changedSourceFiles, organizeImports, uniqueSourceFiles } from "./imports.mjs";

export function printImportsUsage(stream = process.stderr) {
	stream.write("Usage: refarm-task imports [--check] [--all|file...]\n");
	stream.write("\n");
	stream.write(
		"Defaults to changed source files from git. Skips dist/, build/, .turbo/, node_modules/, and .d.ts.\n",
	);
}
...
export function runImportsCommand(argv = process.argv.slice(2), options = {}) {
	...
	if (all) {
		stderr.write(
			"Use explicit files or changed-file mode; --all is intentionally not implemented for this repo.\n",
		);
		return 1;
	...
	if (changed.length === 0) {
		stdout.write(
			`Imports already organized (${selected.length} file${selected.length === 1 ? "" : "s"} checked).\n`,
		);
		return 0;
	}
	...
	if (check) {
		stderr.write(
			`Imports need organizing in ${changed.length} file${changed.length === 1 ? "" : "s"}.\n`,
		);
		return 1;
	}
