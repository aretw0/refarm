...
async function main() {
	...
	if (statusLine && statusLine.length > 0) {
		console.error(
			"❌ Your working tree is not clean. Please commit or stash your changes before starting a new task.",
		);
		process.exit(1);
	}
	...
	if (!hasCli) {
		console.log(
			`⚠️ ${gitUrlAdapter.cliName} not found or not authenticated. Some automated tracking features will be limited.\n`,
		);
	}
	...
	if (phase === "feature") {
		if (hasCli) {
			const inputIssueId = await question(
				`Enter ${gitUrlAdapter.cliName} Issue ID (leave blank to create one): `,
			);
			if (inputIssueId) {
				try {
				...
				const createIssue = await question(
					"No Issue ID provided. Create a new feature request issue? (Y/n): ",
				);
				if (createIssue.trim().toLowerCase() !== "n") {
					...
					if (title) {
						...
						try {
							const newIssueUrl = gitUrlAdapter.issue.create(
								`[Feature]: ${title}`,
								`kind:enhancement,phase:sdd`,
								`Initiated via task:start`,
							);
	...
	console.log(
		`Run '${packageScriptCommand("task:finish").display}' when you're done with your BDD/TDD/DDD cycle.`,
	);
	process.exit(0);
}
