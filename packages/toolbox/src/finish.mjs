...
async function main() {
	...
	if (currentBranch === "main" || currentBranch === "develop") {
		console.error(
			"❌ You are currently on the main or develop branch. Please checkout your feature branch.",
		);
		process.exit(1);
	}
	...
	try {
		...
		console.error(
			"⚠️ Changeset generation skipped or failed. Be careful, a changeset is required for production patches.",
		);
	}
	...
	if (status && status.length > 0) {
		...
		if (useAtomicFlow.trim().toLowerCase() !== "n") {
			const activeGroups = Object.values(groupChanges(getStatusLines())).filter(
			...
			if (autoCommit.trim().toLowerCase() !== "n") {
				...
				const desc = branchParts[1]
					? branchParts[1].replace(/-/g, " ").replace(/^\d+-/, "")
					: "updates";
	...
	try {
		...
		if (openPR.trim().toLowerCase() !== "n") {
			const prTitle = issueId
				? `finish: work on #${issueId}`
				: `feat: merge ${currentBranch.split("/").pop()}`;
