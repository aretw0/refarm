...
export function getFileDiff(path) {
	try {
		...
		const statusLine = execSync(`git status --porcelain -- "${path}"`, {
			encoding: "utf-8",
		}).trim();
...
export function extractSignals(path, diff) {
	...
	if (
		d.includes('"paths"') ||
		d.includes("paths:") ||
		d.includes("homestead/") ||
		d.includes("tractor/")
	)
		signals.add("tsconfig-paths");
	if (d.includes("homestead/ui") || d.includes("homestead/sdk")) signals.add("homestead-subpath");

	const exportRegex =
		/^\+\s*export\s+(?:async\s+)?(?:const|function|class|type|interface|enum)\s+(\w+)/gm;
	let expMatch;
	...
	if (/\bvi\.(?:mock|fn|spyOn|doMock)\s*\(/.test(d) || /from\s+["']vitest["']/.test(d))
		signals.add("test-mock");
	if (/^\+\s*(?:describe|it|test|bench)\s*\(/m.test(d)) signals.add("test-suite");
	if (
		path.endsWith("package.json") &&
		(d.includes("overrides") || d.includes("flatted") || d.includes("audit"))
	)
		signals.add("security");

	// Barn / plugin lifecycle
	if (d.includes("installPlugin") || d.includes("uninstallPlugin") || d.includes("listPlugins"))
		signals.add("barn-api");
	if (d.includes("integrity") || d.includes("sha256") || d.includes("SHA-256"))
		signals.add("barn-integrity");
	...
	if (!scope && (path === "package-lock.json" || path === "package.json"))
		signals.add("infra:root-pkg");
...
export function deriveCommitMessage(groupId, items) {
	...
	const scopes = [...allSignals]
		.filter((s) => s.startsWith("scope:"))
		.map((s) => s.replace("scope:", ""));
	const primaryScope = scopes.length === 1 ? scopes[0] : "";
	const scopePrefix = primaryScope ? `(${primaryScope})` : "";

	const getDeps = () =>
		[...allSignals].filter((s) => s.startsWith("dep:")).map((s) => s.replace("dep:", ""));
	const getExports = () =>
		[...allSignals].filter((s) => s.startsWith("export:")).map((s) => s.replace("export:", ""));
	...
	switch (groupId) {
		...
		case "test_cleanup": {
			...
			const parts = [];
			if (allSignals.has("plugin-api-rename"))
				parts.push("update bench calls to use plugins.findByApi");
		...
		case "barn_impl": {
			...
			if (!detail && allSignals.has("barn-api"))
				detail = "implement installPlugin, listPlugins, uninstallPlugin";
		...
		default: {
			if (groupId.startsWith("scope:")) {
				...
				const parts = [];
				let type = "chore";
				const hasSourceChanges = items.some(
					(item) =>
						item.path.includes("/src/") &&
						!item.path.includes(".test.") &&
						!item.path.includes(".bench."),
				);
				...
				if (allSignals.has("test-file")) {
					parts.push(
						`clean up tests ${exports.length > 0 ? `for ${exports.join(", ")}` : ""}`.trim(),
					);
					type = "fix";
				}
...
export function groupChanges(changes, getDiffFn = getFileDiff) {
	...
	for (const item of preprocessed) {
		...
		if (
			path.includes("git-atomic") ||
			path.includes("git-commit") ||
			path.includes("toolbox/src/") ||
			path.includes("toolbox/test/")
		) {
			...
			const isRust =
				allScopeSignals.has("rust-config") ||
				allScopeSignals.has("rust-src") ||
				allScopeSignals.has("rust-lock") ||
				allScopeSignals.has("wasm-target") ||
			...
			signals.has("infra:turbo") ||
			signals.has("infra:tsconfig") ||
			signals.has("infra:vitest")
		) {
...
export function runCLI() {
	...
	try {
		...
		statusOutput
			...
			.forEach((line) => changes.push(line));
	} catch {}
