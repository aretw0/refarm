	...
	if (mode === "status") {
		...
		for (const pkg of allPackages) {
			...
			const isSrcPath = (str) => {
				...
				return (
					p.startsWith("src/") || p.startsWith("test/") || p.endsWith(".ts") || p.endsWith(".mts")
				);
			};
			...
			if (pkg.type === "apps") {
				...
				isSrc =
					isSrcPath(pkgJson.main) || isSrcPath(pkgJson.types) || checkEntryPoints(pkgJson.exports);
			}
				...
				for (const key in obj) {
					if (typeof obj[key] === "string") {
						if (mode === "src") {
							...
							if (
								obj[key].includes("src/") ||
								obj[key].includes("test/") ||
								obj[key].endsWith(".ts") ||
								obj[key].endsWith(".mts")
							) {
