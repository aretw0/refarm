...
export function parseCommitAutoOptions(argv = process.argv, env = process.env) {
	return {
		strictImportant: argv.includes("--strict-important") || isEnabled(env.GIT_COMMIT_AUTO_STRICT),
	};
}
...
export async function resolveCommitMessage(group, suggestedMessage, options = {}) {
	...
	const requiresManualMessage = importance.important || (strictImportant && lowConfidenceGroup);
	...
	while (true) {
		...
		const answer = (await askQuestion(prompt, readlineInterface)).trim();
		...
		if (answer.length > 0) {
			if (strictImportant && isGenericCommitMessage(answer)) {
				console.log("Message too generic for strict mode. Be specific about what changed.");
				continue;
			}
			return answer;
		}
