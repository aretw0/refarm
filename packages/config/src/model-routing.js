...
export function modelCredentialStatus(provider, tokens = {}, env = defaultEnv()) {
	...
	if (
		oauthProvider &&
		(!normalizedProvider || oauthProvider.toLowerCase() === normalizedProvider)
	) {
...
export function modelRouteTokenUpdate(scope, modelRef, tokens = {}) {
	if (scope === "default") {
		...
		const providerChanged =
			storedProvider !== undefined && nextProvider !== undefined && storedProvider !== nextProvider;
		return {
...
export function effectiveModelRouteForScope(tokens = {}, scope, { env = defaultEnv() } = {}) {
	...
	const storedModelId = sameStoredProvider
		? (stringValue(tokens.modelId) ?? stringValue(tokens.model))
		: undefined;
	const defaultModelId = envModelId ?? storedModelId;
