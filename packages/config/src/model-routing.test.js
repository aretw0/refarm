...
describe("model routing config", () => {
	...
	it("reports model credential status", () => {
		expect(modelCredentialStatus("openai", {}, { OPENAI_API_KEY: "sk-env" })).toEqual({
			state: "env",
			envKey: "OPENAI_API_KEY",
		});
		expect(modelCredentialStatus("openai", { modelApiKey: "sk-silo" }, {})).toEqual({
			state: "silo-api-key",
			envKey: "OPENAI_API_KEY",
		});
		expect(
	...
	it("does not treat stored credentials as usable for another provider", () => {
		expect(
			modelCredentialStatus("gemini", { modelProvider: "openai", modelApiKey: "sk-openai" }, {}),
		).toEqual({ state: "missing", envKey: "GEMINI_API_KEY" });
		expect(
			hasUsableModelCredential("gemini", { modelProvider: "openai", modelApiKey: "sk-openai" }, {}),
		).toBe(false);
	});
	...
	it("parses known provider/model refs with nested model ids", () => {
		expect(parseModelRef("together/meta-llama/Llama-3.3-70B-Instruct-Turbo", undefined)).toEqual({
			provider: "together",
			modelId: "meta-llama/Llama-3.3-70B-Instruct-Turbo",
		});
		expect(parseModelRef("openrouter/anthropic/claude-sonnet-4.6", undefined)).toEqual({
			provider: "openrouter",
	...
	it("treats any explicit slash as provider/model for custom providers", () => {
		expect(parseModelRef("vllm/Qwen3-Coder-480B-A35B-Instruct", "openai")).toEqual({
			provider: "vllm",
