...
describe("package manager config", () => {
	it("honors REFARM_PACKAGE_MANAGER as an operator override", () => {
		expect(detectPackageManager({ env: { [PACKAGE_MANAGER_OVERRIDE_ENV_VAR]: " bun " } })).toBe(
			"bun",
		);
		expect(
			packageScriptCommand("test", { env: { [PACKAGE_MANAGER_OVERRIDE_ENV_VAR]: " bun " } }),
		).toMatchObject({
			packageManager: "bun",
	...
	it("describes ignored invalid REFARM_PACKAGE_MANAGER overrides", () => {
		...
		expect(
			packageManagerOverrideDiagnostic({ [PACKAGE_MANAGER_OVERRIDE_ENV_VAR]: "pnpm" }),
		).toBeNull();
	...
	it("formats dev dependency install commands for each supported manager", () => {
		expect(
			packageAddDevCommand("turbo", { env: { REFARM_PACKAGE_MANAGER: "pnpm" } }),
		).toMatchObject({
			command: pmCommand("pnpm"),
		...
		expect(
			packageAddDevCommand("turbo", { env: { REFARM_PACKAGE_MANAGER: "yarn" } }),
		).toMatchObject({
			command: pmCommand("yarn"),
	...
	it("formats audit commands with optional severity for each supported manager", () => {
		...
		expect(
			packageAuditCommand({ env: { REFARM_PACKAGE_MANAGER: "npm" }, auditLevel: "moderate" }),
		).toMatchObject({
			command: pmCommand("npm"),
		...
		expect(
			packageAuditCommand({ env: { REFARM_PACKAGE_MANAGER: "yarn" }, auditLevel: "high" }),
		).toMatchObject({
			command: pmCommand("yarn"),
			args: pmArgs("yarn", ["npm", "audit", "--severity", "high"]),
			display: "yarn npm audit --severity high",
		...
		expect(
			packageAuditCommand({ env: { REFARM_PACKAGE_MANAGER: "bun" }, auditLevel: "critical" }),
		).toMatchObject({
			command: pmCommand("bun"),
	...
	it("formats workspace publish dry-run commands for each supported manager", () => {
		expect(
			packageWorkspacePublishDryRunCommand({ env: { REFARM_PACKAGE_MANAGER: "pnpm" } }),
		).toMatchObject({
			command: pmCommand("pnpm"),
		...
		expect(
			packageWorkspacePublishDryRunCommand({ env: { REFARM_PACKAGE_MANAGER: "npm" } }),
		).toMatchObject({
			command: pmCommand("npm"),
		...
		expect(
			packageWorkspacePublishDryRunCommand({ env: { REFARM_PACKAGE_MANAGER: "yarn" } }),
		).toMatchObject({
			command: pmCommand("yarn"),
		...
		expect(
			packageWorkspacePublishDryRunCommand({ env: { REFARM_PACKAGE_MANAGER: "bun" } }),
		).toMatchObject({
			command: pmCommand("bun"),
