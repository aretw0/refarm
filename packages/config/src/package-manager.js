...
export function packageManagerSpawnCommand(packageManager, args = [], platform = process.platform) {
...
export function createPackageScriptCommand({
	...
	switch (packageManager) {
		case "pnpm": {
			const pnpm = packageManagerSpawnCommand(packageManager, [
				"-C",
				commandCwd,
				"run",
				script,
		...
		case "npm": {
			const npm = packageManagerSpawnCommand(packageManager, [
				"--prefix",
				commandCwd,
				"run",
				script,
		...
		case "yarn": {
			const yarn = packageManagerSpawnCommand(packageManager, [
				"--cwd",
				commandCwd,
				"run",
				script,
		...
		case "bun": {
			const bun = packageManagerSpawnCommand(packageManager, [
				"--cwd",
				commandCwd,
				"run",
				script,
...
export function packageAuditCommand({
	...
	switch (packageManager) {
		...
		case "npm":
		case "bun": {
			const command = packageManagerSpawnCommand(packageManager, [
				"audit",
				...(auditLevel ? [`--audit-level=${auditLevel}`] : []),
			]);
			return {
				packageManager,
		...
		case "yarn": {
			const yarn = packageManagerSpawnCommand(packageManager, [
				"npm",
				"audit",
				...(auditLevel ? ["--severity", auditLevel] : []),
			]);
			return {
				packageManager,
				command: yarn.command,
