...
export async function runStorageV1Conformance(provider) {
    const failures = [];
    ...
    try {
        ...
        if (!got) {
            ...
            if (got.type !== sample.type)
                failures.push("get() returned wrong type");
        }
    }
    catch (error) {
    ...
    try {
        ...
        if (!hasSample)
            failures.push("query() did not return inserted row");
    }
    catch (error) {
    ...
    try {
        ...
        if (afterDelete !== null)
            failures.push("delete() did not remove row");
    }
    catch (error) {
