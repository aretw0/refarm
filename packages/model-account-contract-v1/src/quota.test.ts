 ...
 * `GET https://api.github.com/copilot_internal/user` with the operator's two real Copilot
 * accounts. A fixture someone wrote from a doc would not have contained the trap in
 * `unlimited` meters, because nobody would think to write `remaining: 0` there.
 */
const CORPORATE = {
	login: "redacted",
...
describe("isMeterExhausted", () => {
	...
	it("calls the operator's corporate seat NOT exhausted at 30 percent", () => {
		const quota = readAccountQuota(CORPORATE);
		expect(isMeterExhausted(quota.meters.premium_interactions!)).toBe(false);
	});
});

describe("meteredEntries", () => {
	it("returns only the meters a denominator can come from", () => {
		expect(meteredEntries(readAccountQuota(CORPORATE)).map(([id]) => id)).toEqual([
			"premium_interactions",
		]);
	});
});
