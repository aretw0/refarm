...
//
// Unlike the jco-based plugins (which build their manifest from a contract package),
// lsp-code-ops authors its manifest by hand in ./plugin.json. This script takes that
// authored manifest, strips the `_note` authoring comment, and stamps the two fields
// that only exist once the .wasm is built:
...
//
// farmhand's bundleInstallPlugin RE-stamps entry+integrity at install time (to an
// absolute file:// path under the plugins dir); this dist manifest is what makes the
// component self-sufficient for DIRECT loading (dev, harness, vault-style tests).
//
...
// Start from the authored manifest; drop the `_note` (authoring-only) and stamp the
// build-time fields. The entry sits next to plugin.json in dist/, so it is relative.
