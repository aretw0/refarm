...
describe("defineCapabilityHost", () => {
	it("lets a white-label app declare extensions once and get registry, CLI, HTTP and base status", async () => {
		...
		const actionEnvelope = (await actions.run({
			args: {},
			options: {},
			json: true,
		})) as unknown as {
	...
	it("lets a host declare a records review queue without manifest plumbing", () => {
				...
				units: ({ recordReviewQueueUnit, hostCommand }) => [
					recordReviewQueueUnit({
						...
						pendingAction: {
							...
							label: "Verify the draft credential",
							intent: "wallet:verify",
							command: hostCommand(["records", "correct", "record:draft", "verified", "--apply"]),
							primary: true,
						},
					}),
				],
		...
		expect(model.nextCommands).toEqual(["dgk records correct record:draft verified --apply"]);
		expect(host.surfaceActions().map((action) => action.id)).toEqual(["verify-draft-credential"]);
	});
	...
	it("builds several primary surface actions from verb declarations", () => {
		...
		expect(
			host.surfaceActions().map((action) => ({
				id: action.id,
	...
	it("builds serve output URLs from the host serve options", () => {
		expect(
			buildCapabilityHostServeInfo(4321, {
...
describe("capability host CLI helpers", () => {
	it("detects direct and bin-shimmed CLI entrypoints", () => {
		expect(
			isCapabilityHostCliEntrypoint("file:///repo/examples/wallet-t2/dist/cli.js", {
		...
		expect(
			isCapabilityHostCliEntrypoint("file:///repo/examples/wallet-t2/src/cli.ts", {
				argv: ["node", "/repo/examples/wallet-t2/dist/cli.js"],
				compiledFileName: "cli.js",
			}),
		).toBe(false);
	...
	it("runs parseAsync only for direct CLI entrypoints and captures failures", async () => {
		...
		expect(
			await runCapabilityHostCli(
				"file:///repo/examples/wallet-t2/src/cli.ts",
				() => ({
					parseAsync,
				}),
				{
		...
		expect(
			await runCapabilityHostCli(
				"file:///repo/examples/wallet-t2/dist/cli.js",
				() => ({
					parseAsync,
				}),
				{
