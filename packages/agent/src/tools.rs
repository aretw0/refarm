...
pub(crate) fn tools_anthropic() -> serde_json::Value {
    serde_json::json!([
        ...
        {"name":"list_tasks","description":"List Task nodes recorded in the CRDT, MOST RECENTLY UPDATED FIRST. Each prompt execution creates one Task. Supports optional filtering by status (active/done/failed/blocked) and context_id (session). Note the clock: this orders by when a Task was last TOUCHED, while the operator-facing GET /tasks orders by when it was CREATED, so the two answer \"the newest N\" differently for any Task whose status changed.",
...
pub(crate) fn tools_openai() -> serde_json::Value {
    serde_json::json!([
        ...
        {"type":"function","function":{"name":"list_tasks","description":"List Task nodes from the CRDT, most recently UPDATED first (not most recently created). Each prompt creates one Task. Filter by status or context_id.",
