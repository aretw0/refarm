...
describe("entry support helpers", () => {
	...
	it("evaluates runtime compatibility", () => {
		...
		expect(evaluateEntryRuntimeCompatibility("./plugin.cjs", "browser")).toEqual({
			runtime: "browser",
			format: "cjs",
			supported: false,
		});
	...
	it("throws explicit runtime compatibility errors", () => {
		expect(() => assertEntryRuntimeCompatibility("./plugin.css", "node")).toThrow(
			"entry must be a .js/.mjs/.cjs or .wasm path",
		);
		expect(() => assertEntryRuntimeCompatibility("./plugin.cjs", "browser")).toThrow(
			"entry format .cjs is not supported in browser runtime",
		);
		expect(() => assertEntryRuntimeCompatibility("./plugin.wasm", "browser")).toThrow(
