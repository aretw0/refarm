...
import { type DsTheme, REQUIRED_TOKENS } from "./contract.js";
...
/**
 * Emit the `builtin-themes.generated.ts` module: the built-in themes as surface-neutral `DsTheme` objects
 * (base, flattened from their DTCG source), as inline TS literals — NOT a runtime JSON import, so it
 * bundles for any surface (web, TUI, agent) with no import-attribute fragility. Tab-indented and
 * eslint-disabled so the generated file is lint-clean. Guarded by tokens-builtin.test.ts (the runtime
 * object must deep-equal the flattened source, so a stale generated file is caught).
 */
export function emitBuiltinThemesModule(themes: Record<string, DsTheme>): string {
	const literal = JSON.stringify(themes, null, "\t");
	return [
		...
		`export const BUILTIN_THEMES: Record<string, DsTheme> = ${literal};`,
		"",
	].join("\n");
}
