...
function snapshot(overrides: Partial<PressureSnapshot> = {}): PressureSnapshot {
	return {
		queueDepth: 0,
		inFlight: 0,
		cancelRequests: 0,
...
function window(overrides: Partial<PressureWindow> = {}): PressureWindow {
	return {
		windowMinutes: 15,
...
describe("pressure contract", () => {
	...
	it("returns reusable pressure recommendations without app commands", () => {
		expect(buildPressureRecommendations(["saturation:queue", "custom:diagnostic"])).toEqual([
			{
