...
const DEFAULT_EMBEDDED_CONFIG_CANDIDATES = ["refarm.config.json", ".refarm/config.json"];
...
export function validatePolicy(policy) {
	...
	assert.ok(allowedModes.includes(policy.mode), `mode must be one of: ${allowedModes.join(", ")}`);
	...
	for (const phase of policy.phases) {
		...
		assert.equal(
			typeof phase.required,
			"boolean",
			`phase.required must be boolean for ${phase.id}`,
		);
		assert.ok(
			typeof phase.riskWeight === "number",
			`phase.riskWeight must be numeric for ${phase.id}`,
		);
	}
	...
	for (const provider of policy.providers) {
		...
		if (
			provider.supportsPublish &&
			(!Array.isArray(provider.publishCommands) || provider.publishCommands.length === 0)
		) {
			throw providerPolicyError(
	...
	for (const profile of policy.packageProfiles || []) {
		...
		if (profile.risk !== undefined && !ALLOWED_PROFILE_RISKS.includes(profile.risk)) {
			throw new Error(
				`package profile risk must be one of: ${ALLOWED_PROFILE_RISKS.join(", ")} for ${profile.id}`,
			);
		}

		if (profile.surface !== undefined && !ALLOWED_RELEASE_SURFACES.includes(profile.surface)) {
			throw new Error(
				`package profile surface must be one of: ${ALLOWED_RELEASE_SURFACES.join(", ")} for ${profile.id}`,
			);
		}

		if (profile.bump !== undefined && !allowedProfileBumps.includes(profile.bump)) {
			throw new Error(
				`package profile bump must be one of: ${allowedProfileBumps.join(", ")} for ${profile.id}`,
			);
		}
	...
	for (const block of policy.surfaceBlocks || []) {
		...
		if (!ALLOWED_RELEASE_SURFACES.includes(block.surface)) {
			throw new Error(
				`surfaceBlocks.surface must be one of: ${ALLOWED_RELEASE_SURFACES.join(", ")}`,
			);
		}
	...
	for (const selection of selections) {
		...
		if (selection.audienceBoundary !== undefined) {
			validateAudienceBoundary(
				selection.audienceBoundary,
				`selection audienceBoundary for ${selection.id}`,
			);
		}
	}
	...
	if (policy.defaultSelection && !selectionIds.has(policy.defaultSelection)) {
		throw new Error(
			`defaultSelection does not match a declared selection: ${policy.defaultSelection}`,
		);
	}
...
function resolveCandidatePackages({ cwd, packageNames, policy, profileTags = [] }) {
	...
	const explicitPackageNames = Array.isArray(packageNames) ? packageNames.filter(Boolean) : [];
	let candidates =
...
export function resolvePolicySelection(policy, selectionId = "default") {
	const selections = Array.isArray(policy?.selections) ? policy.selections : [];
	const resolvedId = selectionId === "default" ? policy?.defaultSelection : selectionId;
...
function topologicalOrder(candidates, allPackages) {
	...
	if (order.length !== candidates.length) {
		const remaining = [...indeg.entries()].filter(([, degree]) => degree > 0).map(([name]) => name);
		throw new Error(
			`Topological order unresolved (cycle or malformed dependency graph): ${remaining.join(", ")}`,
		);
	}
...
export function buildReleasePlan({
	...
	const selection =
		profileTags.length > 0
			? null
			: selectionId
				? requirePolicySelection(policy, selectionId)
				: null;
	const resolvedProfileTags =
		profileTags.length > 0
			? profileTags
	...
	const requiredProviders = policy.providers.filter(
		(provider) => provider.type && provider.supportsPublish,
	);
	const publishIntents = requiredProviders.map((provider) => {
		const versionCommands = Array.isArray(provider.publishCommands) ? provider.publishCommands : [];
		...
		return {
			...
			plan: {
				...
				requiresManualApproval:
					provider.publishRequiresManualApproval ?? policy.mode === "changeset",
			},
		};
	});
...
export function runReleaseGates(
	plan,
	{ cwd = process.cwd(), dryRun = false, onlyRequired = false } = {},
) {
	const results = [];
...
export function formatPlan(plan) {
	if (!plan?.ok) {
		const blockerNames =
			(plan.blockers || [])
				.map((entry) => `${entry.name}: ${entry.note || entry.status}`)
				.join("; ") || "no blockers captured";
		return [
...
function hashReleasePlanAuditPayload(payload) {
	return createHash("sha256").update(stringifyReleasePlanAuditPayload(payload)).digest("hex");
}
...
export function releasePlanAcceptance(plan) {
	...
	const surfaces = [
		...new Set(packageProfiles.map((profile) => profile.surface).filter(Boolean)),
	].sort();
...
export function releasePlanPackageProfiles(plan) {
	return (plan.orderedPackages || []).map((entry) => {
		if (!entry?.profile) {
			return {
				id: entry.name,
				risk: null,
