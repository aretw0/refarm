...
describe("WasiImports — tractor-bridge", () => {
  it("store-node: calls storeNode callback and returns 'ok'", async () => {
    ...
    const result = await imports["plugin:host/tractor-bridge"]!["store-node"]!('{"@id":"urn:x:1"}');
  ...
  it("store-node: no storeNode defined → skips and returns 'ok'", async () => {
    ...
    const result = await imports["plugin:host/tractor-bridge"]!["store-node"]!("{}");
    expect(result).toBe("ok");
  });

  it("request-permission: always returns true", async () => {
    const { imports } = makeImports("strict");
    const ok = await imports["plugin:host/tractor-bridge"]!["request-permission"]!(
      "read:storage",
      "needs data",
    );
    expect(ok).toBe(true);
...
// ---------------------------------------------------------------------------
// Drift prevention: every plugin:host/* import in the transpiled agent
...
describe("WasiImports — drift prevention: .jco-dist matches registered imports", () => {
  it("every plugin:host/* import in _refarm_agent.js has a host registration", () => {
    ...
    // Extract all unique 'plugin:host/X' strings from import statements.
    const imported = new Set<string>();
    for (const m of js.matchAll(/from\s+'(plugin:host\/[^']+)'/g)) {
...
// ---------------------------------------------------------------------------
describe("WasiImports — plugin:host/model-bridge", () => {
  beforeEach(() => {
  ...
  it("fails closed when non-ollama provider has no credentials", () => {
    const { imports } = makeImports("strict");
    const modelBridge = imports["plugin:host/model-bridge"]!;
