...
describe("channel effort payload validation", () => {
	it("accepts valid effort payloads", () => {
		expect(isChannelEffortPayload({ direction: "x", tasks: [] as const })).toBe(true);
	});

	it("rejects invalid effort payloads", () => {
		expect(isChannelEffortPayload({ direction: "", tasks: [] })).toBe(false);
		expect(isChannelEffortPayload({ direction: "x", tasks: "not-array" })).toBe(false);
...
describe("channel control-surface adapters", () => {
	it("resolves known and fallback adapters", () => {
		...
		expect(known.adapter.buildSubmitPath("http://127.0.0.1:42001", "matrix")).toBe(
			"http://127.0.0.1:42001/channels/matrix/efforts",
		);
		expect(known.adapter.buildSummaryPath("http://127.0.0.1:42001", "matrix")).toBe(
			"http://127.0.0.1:42001/efforts/summary",
		);

		expect(unknown.channel).toBe("future-irc");
		expect(unknown.adapter.buildListPath("http://127.0.0.1:42001", "future-irc")).toBe(
	...
	it("supports canonical capability checks and registry override", () => {
		...
		expect(resolveChannelControlSurfaceAdapter("matrix").adapter.capabilities.retry).toBe(false);
		...
		expect(resolveChannelControlSurfaceAdapter("matrix").adapter.capabilities.retry).toBe(true);
		expect(() =>
			assertChannelControlCapability(
				resolveChannelControlSurfaceAdapter("matrix").adapter,
				"retry",
		...
		expect(
			hasChannelControlCapability(resolveChannelControlSurfaceAdapter("matrix").adapter, "retry"),
		).toBe(true);
	});
});
