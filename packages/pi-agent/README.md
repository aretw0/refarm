...
- `src/provider_runtime.rs` — provider loop helpers (headers/path/body builders, per-iteration request executors + response parse/extractors, iteration response+phase helpers, iteration-phase parsers, loop-state/loop-plan builders, runner configs + config-driven runners, usage+phase combiners, initial wire-message builders, response-usage/finalization helpers, completion content guards + termination gate helpers, provider-specific completion gates, step-level terminate/advance helpers, generic loop orchestrator + plan-driven orchestration + loop outcomes, dispatch-injectable loop orchestration + shared wasm loop executor, wasm loop runners, generic tool executor + tool-phase executors/recorders + phase advancement helpers, max iter, loop termination, error extraction, wire-message appenders, tool dispatch dedup, argument parse, usage aggregation, executed-call shaping)
