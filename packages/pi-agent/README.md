...
- `src/provider_runtime.rs` — provider loop helpers (headers/path/body builders, initial wire-message builders, response parse/extractors, max iter, loop termination, error extraction, wire-message appenders, tool dispatch dedup, argument parse, usage aggregation, executed-call shaping)
