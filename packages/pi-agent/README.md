...
- `src/provider_runtime.rs` — provider loop helpers (headers/path/body builders, request execution + response parse/extractors, initial wire-message builders, response-usage/finalization helpers, completion content guards, tool execution recorders, max iter, loop termination, error extraction, wire-message appenders, tool dispatch dedup, argument parse, usage aggregation, executed-call shaping)
