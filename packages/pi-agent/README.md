...
- `src/provider_runtime.rs` — provider loop helpers (headers/path/body builders, per-iteration request executors + response parse/extractors, iteration-phase parsers, loop-state builders, initial wire-message builders, response-usage/finalization helpers, completion content guards + termination gate helpers, tool-phase executors/recorders + phase advancement helpers, max iter, loop termination, error extraction, wire-message appenders, tool dispatch dedup, argument parse, usage aggregation, executed-call shaping)
