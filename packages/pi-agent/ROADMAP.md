...
- [x] Extract shared provider-loop helpers into `provider_runtime.rs` (headers/path/body builders, initial wire-message builders, response parse/extractors, loop termination, error extraction, wire-message appenders, dedup dispatch, argument parse, usage aggregation, executed-call shape)
