...
- [x] Extract shared provider-loop helpers into `provider_runtime.rs` (headers/path/body builders, per-iteration request executors + response parse/extractors, initial wire-message builders, response-usage/finalization helpers, completion content guards + termination gate helpers, tool-phase executors/recorders, loop termination, error extraction, wire-message appenders, dedup dispatch, argument parse, usage aggregation, executed-call shape)
