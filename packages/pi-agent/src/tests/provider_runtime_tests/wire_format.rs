...
fn provider_runtime_parse_stream_text_deltas_reads_openai_and_anthropic_payloads() {
    let payloads = vec![
        r#"{"choices":[{"delta":{"content":"he"}},{"delta":{"content":"l"}}]}"#.to_string(),
    ...
    assert_eq!(
        ...
        vec!["he".to_string(), "l".to_string(), "lo".to_string()]
    );
}
...
fn provider_runtime_stream_body_gate_requires_explicit_opt_in() {
    let _guard = super::ENV_LOCK.lock().expect("env lock");
    std::env::set_var(crate::streaming_config::MODEL_STREAM_RESPONSES_ENV, "1");
