...
import { SYNC_CAPABILITY, type SyncErrorCode, type SyncTelemetryEvent } from "./types.js";

/**
 * Type-level proof, not a runtime check: `SyncTelemetryEvent` (declared as a
 * literal interface in types.ts — see the doc comment there for why) must
 ...
 */
export type SyncTelemetryEventShapeProof = Expect<
	TypesAreEqual<
		SyncTelemetryEvent,
		CapabilityTelemetryEvent<
			typeof SYNC_CAPABILITY,
			"connect" | "sync" | "disconnect" | "conflict",
			SyncErrorCode
		>
	>
>;
