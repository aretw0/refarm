...
- Model credential provider selection and OAuth text prompts now route through
  the shared `@refarm.dev/prompt-contract-v1` operator channel instead of being
  hardwired to the app prompt library.
...
- Secret credential entry still uses an app-specific no-echo prompt until the
  shared operator prompt contract grows a secure secret primitive.
