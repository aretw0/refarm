...
| 2 | `quality:v1` checkers (text + ui) | quality-contract-v1 §2 · `packages/ds/src/lint.ts` (`ds-lint:v1`) · design-tells catalog · **plan: `docs/superpowers/plans/2026-07-02-ds-lint-quality-checker-adapter.md`** | ✅ ui adapter **implemented**: `@refarm.dev/ds/quality-checker` wraps `ds-lint:v1` as a `quality:v1` `QualityChecker`; the native text suite remains the text-checker lane | use the adapter in local surfaces and consumer review packets; add more checkers only when a consumer proof needs a new quality domain |
