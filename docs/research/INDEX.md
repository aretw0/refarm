...
KEEP-AUTHORING the surface-neutral token registry, TUI projection, cross-surface (web+TUI+agent) architecture, and shadcn-style code ownership; ADOPT DTCG + Style Dictionary as the token *source* (built-ins are still CSS-authored today), `Intl.RelativeTimeFormat` for Hoje/Ontem, and `role="log"` on the transcript; LEARN-FROM Zag.js's machine+adapter split and Matrix's conversation invariants. 2026-07-19 addendum closes the schema-forms gap (ADOPT Ajv; LEARN JSONForms schema⊕uischema, Adaptive Cards host-agnostic model + its cross-surface-consistency lesson, Vercel tool-call→UI pattern; KEEP-AUTHORING the web+TUI+agent projection) and adds a TUI-composition verdict (KEEP the projection; ADOPT-candidate a layout engine like Yoga/taffy).
