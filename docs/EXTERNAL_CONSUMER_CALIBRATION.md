...
| Writable consumer | `agents-lab`, `vault-seed`, `coop-vault`, `enem`, `arch-engine` | Only when explicitly scoped | Prove that primitives work outside Refarm and keep repo-local policy there. `arch-engine` is the first non-JavaScript producer: it emits `quality:v1` and `artifact:v1` from Python and proves them with the real tarballs (`.refarm/handoff/vault-seed/2026-08-29/receipts/*.arch-engine.json`). |
