...
- **`@refarm.dev/dispatch-surface`** — Transport control-plane adapter primitives (`file`, `http`, `channel:*`) with stable effort envelope and path normalization APIs. Designed as the shared boundary for any CLI, daemon, or UI layer. (→ [Control-plane contract](./superpowers/specs/2026-06-17-dispatch-control-plane-contract.md))
