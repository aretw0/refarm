...
they disagree. Its source family is the consolidation target tracked by the
[diagram catalog](../specs/diagrams/INDEX.md).

![Architecture Layers](../specs/diagrams/architecture-layers.svg)
[View source](../specs/diagrams/architecture-layers.mermaid)
...
- **`@refarm.dev/task-contract-v1`** — Durable work items persisted in the CRDT graph. Usable by agents and humans from the same base schema. Composes with `effort-contract-v1` for dispatch. (→ [design spec](../specs/features/task-contract-v1.md))
- **`@refarm.dev/session-contract-v1`** — Conversation thread primitive for MODEL agents, messaging integrations, and A2A coordination. Pi-agent extends with MODEL-specific fields; base contract is agnostic. (→ [design spec](../specs/features/session-contract-v1.md))
- **`@refarm.dev/dispatch-surface`** — Transport control-plane adapter primitives (`file`, `http`, `channel:*`) with stable effort envelope and path normalization APIs. Designed as the shared boundary for any CLI, daemon, or UI layer. (→ [Control-plane contract](../specs/features/dispatch-control-plane-contract.md))
- **`@refarm.dev/dispatch-surface-rs`** — WIT-first, Rust-native implementation behind the same surface for deterministic and portable transport semantics (Node/wasm parity path under optional fallback).
