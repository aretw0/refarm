...
| 16 | **Content projection (MD/MDX authoring)** — `content-projection` block + future `ds-astro` embed set | implemented package + render-adapter activated | 1 (`source-local`), 4 (`ds`), `records-contract-v1` | `apps/site` authors ≥1 `.mdx` composing a `ds-astro` component, projecting to a valid `records:v1`; `vault-seed` MD machinery converges onto `content-projection` | ◑ phase 1 implemented: `@refarm.dev/content-projection` handles frontmatter, wikilinks, inline Markdown links, and valid `records:v1` output; official `vault-seed` render pressure received on 2026-07-03, so `ds-astro` now needs the package plan + handoff proof ([spec](./superpowers/specs/2026-07-02-content-projection-md-mdx-design.md), [plan](./superpowers/plans/2026-07-03-ds-astro-mdx-render-adapter.md), [research](./research/2026-07-02-content-projection-blocks-md-mdx.md)) |
...
code that moves into `apps/refarm`.
...
plugin-manifest/Barn/Scarecrow policy gates. Packages can bundle `SKILL.md`, guides, references,
themes, and executable extensions together; plugin-manifest remains the distribution/trust boundary.
...
`validations/remote-workspace-control-plane`.
...
### 15. Verification-as-completion — first proof, not product-ready
The peerd/RTK/Terax research converges on a runtime rule: compact observations
help the model, but completion needs source-truth evidence. The first proof is
specified in `specs/features/2026-07-01-verification-as-completion-proof.md`
and implemented in `validations/verification-as-completion`: command observation
with raw evidence recovery, source-hash re-observation, completion gating,
tool-less/keyless delegation, and an existing `context:v1` context map.

This does not extract `tool-observation:v1` yet and does not move behavior into
`apps/refarm`. Extraction waits for dogfood or a second consumer that needs the
same observation/completion contract.
