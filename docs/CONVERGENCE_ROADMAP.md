...
| 16 | **Content projection (MD/MDX authoring)** — `content-projection` block + `ds-astro` embed set | spec + plan | 1 (`source-local`), 4 (`ds`), `records-contract-v1` | `apps/site` authors ≥1 `.mdx` composing a `ds-astro` component, projecting to a valid `records:v1`; `vault-seed` MD machinery converges onto `content-projection` | ▶ design spec landed ([spec](./superpowers/specs/2026-07-02-content-projection-md-mdx-design.md), [research](./research/2026-07-02-content-projection-blocks-md-mdx.md)); plan + build pending |
