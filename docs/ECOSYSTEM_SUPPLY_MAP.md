...
| Reference agent driver | ad hoc agent loops, `dgk` orchestration ideas, worker delegation | `@refarm.dev/cli/capability-index` ✅ + `@refarm.dev/cli/interaction-driver` ✅ + `@refarm.dev/cli/worker-profile` ✅; `@refarm.dev/pi-agent` unheld as a narrow engine package | Supply the embeddable ask/worker/session/tool contracts first. `pnpm run pi-agent:release-proof` gates the built-artifact engine boundary; plugin-manifest, WIT component packaging, third-party authoring, cancellation, observability, and cost ledgers remain separate promotion gates. Consumers keep their own command labels and product UX. |
