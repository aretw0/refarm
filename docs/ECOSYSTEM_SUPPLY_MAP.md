...
| Reference agent driver | ad hoc agent loops, `dgk` orchestration ideas, worker delegation | `@refarm.dev/cli/capability-index` ✅ + `@refarm.dev/cli/interaction-driver` ✅ + `@refarm.dev/cli/worker-profile` ✅; `@refarm.dev/agent` unheld as a narrow engine package | Supply the embeddable ask/worker/session/tool contracts first. `pnpm run agent:release-proof` gates the built-artifact engine boundary; plugin-manifest, WIT component packaging, third-party authoring, cancellation, observability, and cost ledgers remain separate promotion gates. Consumers keep their own command labels and product UX. |
...
| Knowledge/content export | OKF mapping, JSON-LD graph, semantic graph, changelog-as-content, requirement-like records | `@refarm.dev/records-contract-v1` ✅ + `@refarm.dev/records-contract-v1/yaml` candidate + release/artifact envelopes | T3 consumer-proven in `vault-seed`: `records:v1` validates source-linked requirement-like records and notes-to-records projection with graceful degradation. The YAML-LD subpath is implemented, package-tested, and second-consumer proven for YAML front matter; final promotion stays with the release lane. OKF mapping, editorial governance, article conventions, and domain vocabulary stay downstream. |
...
9. Reference-driver contracts advance through the package layer before runtime
   ...
   `runtime-agent`/`agent` execution package remains private until the
