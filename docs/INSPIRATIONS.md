...
- **[wasmCloud (Bytecode Alliance)](https://wasmcloud.com)**: O sistema de computação distribuída sobre WASM Component Model mais battle-tested em produção. O conceito de **Lattice** (hosts como protocolo, não implementação), **Link Definitions** (fiação explícita `componente → provider`), e o modelo de **Providers como componentes de primeira classe** são referências diretas para o design do `tractor`, do `BackendDescriptor`, e do futuro `refarm component dev`. O `wash` CLI é o benchmark de DX para developer tooling de componentes WASM. Contextos diferentes — eles resolvem cloud-native distribuído, nós resolvemos soberania local — mas os primitivos que eles validaram são os mesmos que precisamos dominar. Ver análise em [`docs/research/wasmcloud-analysis.md`](research/wasmcloud-analysis.md).
