...
| Remote workspace control | Active and substrate-proven. Named, shell-free commands reviewed with `remote: true` are projected through `/operations` to Termux and the browser/PWA. `operation-result.v1` carries only bounded, redacted summaries/metrics/findings; stdout and argv never become a remote terminal. The first downstream producer is rcdc5 `code-boundaries` (`66f8d91`), consumed through vendored Refarm packages. | Close the discovered migration footgun: add recorded CLI authorship for `spawnEnv.path`/`spawnEnv.home`, use it in the home scope, and repeat the real rcdc5 operation through the authenticated endpoint. Only after that proof, project the same contracts into Telegram. | Inheriting the daemon's ambient `PATH`; editing config JSON by hand; teaching Refarm rcdc5-specific output; treating SSH, host paths, Tailscale, or a generic remote shell as the abstraction. |
