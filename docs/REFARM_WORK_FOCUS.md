...
| Remote workspace control | Active. Named, shell-free workspace commands run locally through `refarm workspace run`; commands explicitly reviewed with `remote: true` are projected through the device-only `/operations` catalog and `farm-start`. Each accepted start now has an opaque run id and bounded lifecycle status (`running`/`succeeded`/`failed`); the device still sends no argv and receives no terminal stream. Local-only remains the default. | Decide result semantics only with a concrete consumer, then project the shared catalog and lifecycle into the browser/PWA and Telegram. | Treating SSH, mounts, host paths, or Tailscale as the core abstraction; a generic remote shell; remotely admitting a command by implication rather than explicit review. |
