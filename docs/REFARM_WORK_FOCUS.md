...
| Content projection / MD-MDX authoring | Designed, build-pending. Research + design spec landed (`docs/research/2026-07-02-content-projection-blocks-md-mdx.md`, `docs/superpowers/specs/2026-07-02-content-projection-md-mdx-design.md`); flagged by the `vault-seed` consumer. Two blocks: `content-projection` (over `records-contract-v1`, composes with `source-local`) + `ds-astro` (sanctioned MDX embed set over `ds/html`). | Turn the design into the task-by-task plan; build phase 1 (`content-projection`) with a `records-contract-v1` conformance test; graduate the `vault-seed` MD machinery onto it as the consumer proof. | Naming it a `source-*` adapter (acquisition is `source-local`); coupling `ds` core to Astro (bind only in `ds-astro`); folding in the DS composition guardrail (separate graduation). |
