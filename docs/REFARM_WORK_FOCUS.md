...
| Remote workspace control | Active. Named, shell-free workspace commands run locally through `refarm workspace run`; commands explicitly reviewed with `remote: true` are projected through the scoped `/operations` catalog and `farm-start`. Device credentials carry full authority; verified browser sessions may receive only `operation:read` and `operation:start`. Each accepted start has an opaque run id and bounded lifecycle status (`running`/`succeeded`/`failed`); the surface still sends no argv and receives no terminal stream. Local-only remains the default. | Project the shared catalog and lifecycle into the browser/PWA and Telegram; decide richer result semantics only with a concrete consumer. | Treating SSH, mounts, host paths, or Tailscale as the core abstraction; a generic remote shell; remotely admitting a command by implication rather than explicit review. |
