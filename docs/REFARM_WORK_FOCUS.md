...
| Content projection / MD-MDX authoring | Phase 1 implemented, selected, and downstream-proven. `@refarm.dev/content-projection` projects frontmatter, wikilinks, and inline Markdown links into valid `records:v1`; `ds-astro` remains a separate future render adapter over `ds/html`. | Use concrete gaps from the official proof to decide whether `ds-astro` or more projection helpers are needed. | Naming it a `source-*` adapter (acquisition is `source-local`); coupling `ds` core to Astro (bind only in `ds-astro`); folding in the DS composition guardrail (separate graduation). |
