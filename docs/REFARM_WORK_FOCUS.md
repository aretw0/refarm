...
| Remote workspace control | Active. Named, shell-free workspace commands run locally through `refarm workspace run`; commands explicitly reviewed with `remote: true` are projected through the scoped `/operations` catalog and `farm-start`. Device credentials carry full authority; verified browser sessions receive only `operation:read` and `operation:start`. The browser projection now lists, starts and follows the bounded lifecycle through reusable `operation-web-v1`, localized and rendered with canonical `ds/html`; Termux uses the same routes. No surface sends argv or receives a terminal stream. Local-only remains the default. | Prove one real workspace operation from the PWA, then project the same catalog and lifecycle into Telegram; decide richer result semantics only with a concrete consumer. | Treating SSH, mounts, host paths, or Tailscale as the core abstraction; a generic remote shell; remotely admitting a command by implication rather than explicit review; app-local HTML components that bypass `ds/html`. |
