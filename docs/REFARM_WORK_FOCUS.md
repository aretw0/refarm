...
| Remote workspace control | Active. Named, shell-free workspace commands run locally through `refarm workspace run`; commands explicitly reviewed with `remote: true` are projected through the device-only `/operations` catalog and `farm-start`. The device sends only an opaque id, and the node re-enters the same allowlist. Local-only remains the default. | Add status/result visibility for remotely started workspace operations, then project the shared catalog into the browser/PWA and Telegram. | Treating SSH, mounts, host paths, or Tailscale as the core abstraction; a generic remote shell; remotely admitting a command by implication rather than explicit review. |
