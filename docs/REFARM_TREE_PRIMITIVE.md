...
emit `schemaVersion: 1` directly at each producer and use an explicit `operation`
discriminator (`list`, `show`, `preview`, or `fork`). `preview` emits a dry-run envelope that recommends
