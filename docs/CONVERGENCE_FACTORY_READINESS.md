...
| T2 identity/credentials | reference-proven, release-profiled, not release-selected | `@refarm.dev/identity-heartwood` supplies real Heartwood-backed Ed25519 `identity:v1`; `@refarm.dev/credentials-contract-v1` composes issue/verify/present/store/list over `identity:v1` + `storage:v1`; `pnpm run sovereign-citizen:reference:test` proves the sanitized issue -> verify -> present -> wallet flow; both profiles are held as `reference-hold` | issuer trust registry, credential schemas, downstream wallet UX, revocation policy, and selected consumer proof remain gated before handoff promotion |
...
| `vault-seed` release lane | dry-run validated + T3 consumer-proven | `vault-seed-ready` selection lives in versioned `refarm.config.json`; after the T3 proof the lane reports `acceptance.status: "accepted"` with 13 packages and 35 required checks, including the selected `records-contract-v1` unit gate that covers the YAML subpath candidate; the 2026-06-30 local handoff ready packet remains historical at `.refarm/handoff/vault-seed/2026-06-30/manifest.json` with `distributionEvidence.state: "local-handoff-ready"` and no stale tarball/build-output issues; `scripts/ci/test-vault-seed-release-consumer.mjs` proves generated-vault `@refarm.dev/*` dependencies are covered by `vault-seed-ready`; the lane selects light surfaces such as `@refarm.dev/ds/html` and `@refarm.dev/process-handoff` instead of full Homestead/CLI closures; `vault-seed` now proves `source-web` fixture -> `records:v1` -> `enrichment:v1` composition with 16/16 consumer-contract tests | remaining non-T3 official downstream assimilation proofs are still pending; rematerialize the active 13-package handoff before publication/push |
