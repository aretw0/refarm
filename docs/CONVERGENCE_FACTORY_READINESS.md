...
| 6 native skills | activation-gated | taxonomy; native activation spec+plan; `@refarm.dev/skill-contract-v1` parses `SKILL.md` into `SkillManifestV1`, verifies loaded source integrity, and prepares a host-policy-checkable invocation plan without executing skills | plugin-manifest skill surface + engine dogfood gate not present |
