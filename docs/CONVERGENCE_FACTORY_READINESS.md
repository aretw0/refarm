...
|---|---|---|---|
| 1 librarian `source:v1` | **implemented** | `@refarm.dev/source-contract-v1`, `@refarm.dev/source-git`, `@refarm.dev/source-local`, conformance, build, `test:capabilities`, release-policy held profiles, and librarian smoke re-verified 2026-06-29 | `source-dispatch` remains item 7 and activates only when consumed |
...
| 7 librarian completion | correctly deferred | source:v1 base contract plus held source package profiles | waits for dispatch consumer; live-tree reads use `@refarm.dev/source-local` but still need consumer proof before handoff promotion |
