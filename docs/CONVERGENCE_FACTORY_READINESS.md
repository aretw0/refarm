...
| 4a `ds` tokens | **implemented + downstream-proven** | contract, scoped tokens, 4 themes, theme-conformance, component classes, CSS exports, and quality adapter; focused package gate re-verified 2026-06-26; official downstream proof received through the 2026-07-03 `consumer-ready` handoff | broad steward/push gate and public publication still wait on release lane |
| 4b `ds/html` | **implemented + downstream-proven** | DS-owned helper API, build-free boundary, consumer proof, package files constraint, and official downstream site/admin proof through the 2026-07-03 `consumer-ready` handoff | public publication still waits on release lane |
...
| T2 identity/credentials | consumer-pulled into `consumer-ready` | `@refarm.dev/credentials-contract-v1` exposes issue/policy-driven verify/present/store/list/remove/revoke over `identity:v1` + `storage:v1`; `@refarm.dev/identity-contract-v1` and `@refarm.dev/storage-contract-v1` are selected as support contracts; `@refarm.dev/identity-heartwood` supplies real Heartwood-backed Ed25519 issuer/holder signatures; `@refarm.dev/storage-memory` supplies the volatile wallet implementation; `pnpm run sovereign-citizen:reference:test` proves the sanitized issue -> policy checks -> present -> wallet flow, including trusted issuer, `trustSelf`, holder-binding, and local signed status-list revocation evidence | issuer authorities, credential schemas, trust registry sources, downstream wallet UX, remote status-list distribution, and production persistence/encryption remain downstream |
| 8 consumer bridges | partially activated | 8a Refarm-side package proof and handoff are complete, with Silo storage helpers, protection envelope, and storage/identity closure split folded back from the `vault-seed` proof; 8b has the `channel-policy-v1` spec/package slice; 8c has the `process-handoff` leaf -> artifact provenance proof | product adoption of the Silo-backed credential bridge remains downstream; broader bridge publication waits on the release lane |
...
| `vault-seed` release lane | official downstream proof received | `consumer-ready` selection lives in versioned `refarm.config.json`; after the T2 credentials pull, agent-demo public-surface proof, `quality:v1` pull, `content-projection` pull, `local-surface` pull, `ds-astro` render-adapter pull, and `health` toolchain-auditor pull the lane reports `acceptance.status: "accepted"` with 23 packages and 72 required checks, including the selected `records-contract-v1` unit gate that covers the YAML subpath candidate, the `sovereign-citizen:reference:test` round-trip proof, generic Markdown/MDX note projection for frontmatter, wikilinks, and inline Markdown links, the sanctioned Astro/MDX DS embed set, and the `ToolchainAuditor` substrate for `dgk doctor` hollowing; the current 2026-07-03 local handoff ready packet is materialized at `.refarm/handoff/vault-seed/2026-07-03/manifest.json` with `distributionEvidence.state: "local-handoff-ready"`, 23 tarballs, `manifest.md`, and no missing/extra tarball issues (the 2026-07-02 directory is a superseded partial packet without a manifest); the official downstream checkout reported successful vendor SHA-256 verification for the previous 21-tarball packet, focused consumer proof coverage for quality/content/source/records/enrichment surfaces, records manifest generation with 93 valid records, Astro site build with 86 pages, and package smoke success while keeping product labels, vault UX, provider adapters, notebooks, publication copy, and private POC specifics downstream; `scripts/ci/test-vault-seed-release-consumer.mjs` proves generated-vault `@refarm.dev/*` dependencies are covered by `consumer-ready`; the lane selects light surfaces such as `@refarm.dev/ds/html` and `@refarm.dev/process-handoff` instead of full Homestead/CLI closures; `vault-seed` proves `source-web` fixture -> `records:v1` -> `enrichment:v1` composition with 16/16 consumer-contract tests, and the official credentials consumer-contract proof vendors credentials + identity-heartwood + storage-memory and passes the Heartwood-signed issue -> verify -> present -> wallet conformance | publication still waits on develop stabilization, GitHub Actions, release approval, and swapping local `file:vendor/*.tgz` handoff paths for npm versions |
...
| 8b channel policy (`contacts` + `rate-limiter` + receipts) | **downstream-proven package slice**: `@refarm.dev/channel-policy-v1` covers destinations, rate-limit policy/evidence, delivery state, receipts, dry-run, and review semantics | official `vault-seed` Telegram adapter proof now emits the neutral envelope while keeping API/UX downstream |
| 8c `process-handoff` + artifact provenance | **downstream-proven bridge slice**: `@refarm.dev/process-handoff` process specs validate as `artifact-contract-v1` provenance; `@refarm.dev/cli/process-handoff` remains a compatibility re-export | official `dgk-runner`/`dgk-cli` proof now imports the SDK internally, preserves `dgk` UX, and emits manifests without leaking `dgk` command names upstream |
...
dependencies are covered by the `consumer-ready` release policy. Official downstream adoption
