...
| T2 identity/credentials | reference-proven, not release-selected | `@refarm.dev/identity-heartwood` supplies real Heartwood-backed Ed25519 `identity:v1`; `@refarm.dev/credentials-contract-v1` composes issue/verify/present/store/list over `identity:v1` + `storage:v1`; `pnpm run sovereign-citizen:reference:test` proves the sanitized issue -> verify -> present -> wallet flow | issuer trust registry, credential schemas, downstream wallet UX, revocation policy, and selected consumer proof remain gated before handoff promotion |
