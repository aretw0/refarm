...
| Block imports across `src` | `@refarm.dev/cli` ×158, `config` ×32, `silo` ×8, `homestead` ×6, plus effort/prompt/stream/task/storage/context contracts, windmill, sower, barn, release-engine, runtime, agent, dispatch-surface, health. | App is **pervasively composed** from ~20 blocks. The 28k lines are command orchestration, not hoarded primitives. |
