...
- **rcdc5 migration blueprint** — `rcdc5/docs/OCAMENTO-REFARM.md` (commit `0e12c8c`): the per-package generic↔specific map, migration order, Nexus-publish note. The rcdc5 real migration has begun (planned).

**Next corners (grounded, unordered — take by real friction):**
- **rcdc5 first real consumption** (the migration's first code slice, per the blueprint): `@rcdcp/scraper-playwright` CONSUMES `source-oslc` — vendor the `@refarm.dev/*` tarballs via handoff (like vault-seed), swap the hand-rolled OSLC for the generic toolkit/provider, keep SerproID auth + project URLs downstream. Verify rcdc5's suite green.
