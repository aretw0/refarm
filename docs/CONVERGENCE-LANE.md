...
- **C, rung 1 — DONE**: `examples/reqbench-t3/src/rcdc5-enrichment.ts` + `.parity.test.ts` — reqbench's proven operate-surface (the same `createRulesEnrichmentProvider` the capability-host drives) now runs rcdc5's **REAL** rules (its `.rm-enrichment.json`: CNPJ/CPF/integração) instead of the synthetic `REQ_ENRICHMENT_RULES`, proven **byte-identical** to rcdc5's own `@rcdcp/rm-enrichment` runner (oracle inlined; the space-vs-`\n` multi-source join proven immaterial). Provenance now stamped per tag (`rcdc5.rm-enrichment`), which rcdc5's runner never produced. Sovereign boundary held (rules/`rcdc5/` vocab are a labelled fixture; engine stays generic). Baseline preserved: type-check 0, **reqbench-t3 106 tests + 1 skip** (was 90; +16), no regressions.

**Next corners (grounded, unordered — take by real friction):**
- **C, rung 2 — the operate proof end-to-end**: wire `createRcdc5EnrichmentProvider()` into a capability-host so `<cmd> enrich` dispatches rcdc5's enrichment through the runtime (operator-status, HTTP), modelled on `reqCapabilityBundle`. Then promote toward rcdc5 self-declaring (vendored `capability-host` closure) = "rcdc5 as a refarm app". After C: **B `engine → vault:v1`** (structural: `@rcdcp/extractor-engine` implements the four `vault:v1` verbs), then **A** rebuild almtask's task-gen in rcdc5 (TS) over `task-recurrence`/`task-calendar` using the UST catalog.
- **Scraper convergence** (part of B's neighbourhood): adopt `browser-driver` + a small **Playwright `BrowserSession` adapter** (a reusable 2nd entry alongside `/puppeteer`); reduce rcdc5's `scraper-playwright` to the SerproID login adapter + domain matcher-is-data; the ~80% generic OSLC scraping is `source-web`+`source-oslc` already.
