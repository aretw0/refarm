...
- **Reduce rcdc5's `scraper-playwright`**: wrap its Playwright `context` as a `createPlaywrightSession` `BrowserSession` (its SerproID QR flow → a custom `ensureLoggedIn`), then let `source-web`+`source-oslc` own the ~80% generic OSLC scraping (crawl/401-reauth/pacing/cookie-state) it currently duplicates. Parity-gated, in rcdc5.
