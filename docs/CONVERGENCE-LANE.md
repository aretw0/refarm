...
- **More surgical rcdc5 swaps** (each parity-gated): the primaryText→md flow (rcdc5 keeps raw HTML — needs care, not a clean `oslcPrimaryTextToMarkdown` drop-in); `oslcRequestHeaders` at the full-header call-site; then `engine → vault:v1`, `rm-enrichment → enrichment:v1`. Rebuild almtask's specific task-gen in rcdc5 (TS) over `task-recurrence`/`task-calendar`.
