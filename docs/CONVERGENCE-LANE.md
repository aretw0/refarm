...
- **Wire rcdc5's scraper onto the generic path** (needs a LIVE SerproID login to validate — operator-run): `createSerproBrowserSession` → `createLiveFetch` → `createOslcSourceProvider`, retiring the ~80% of `fetchers.ts` (crawl/401-reauth/pacing/cookie-state) that duplicates `source-web`. Parity-gated, in rcdc5.
