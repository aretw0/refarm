...
`refarm status --action --input` is rejected, and verify `refarm status --action 2`
returns a handled Homestead action invocation envelope. Keep this as local
validation; CI wiring remains deferred while GitHub Actions budget is over
allocation.
