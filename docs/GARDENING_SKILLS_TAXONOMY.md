...
- **`refarm`**: **no executable skill surface yet.** It has a native contract package that parses
  `SKILL.md` into a policy-checkable manifest/invocation plan handoff, plus *engines* — `sower` (scaffold/import), `thresher`
...
`prepareSkillInvocationPlan` as the adapter/host handoff for one source. The
