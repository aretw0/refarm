...
**Status**: Accepted
**Progress**: Implemented + verified. `verify(input, policy?: CredentialVerificationPolicy)` is policy-driven (`packages/credentials-contract-v1/src/types.ts`), the reference provider evaluates every decided field (issuerTrusted incl. dynamic `trustSelf`, validity, requiredClaims, revocation via signed status list, holderBinding), the conformance suite exercises them, and `examples/wallet-t2` consumes `policy` for real: its `verify` verb passes a DEFAULT policy (`{ validity: "required" }` — the always-safe floor above a bare signature check) and a `--strict` mode that additionally requires revocation-status + issuer-trust (`strictWalletVerifyPolicy` in `examples/wallet-t2/src/credentials.ts`); a deployment pins its trusted civic issuers via the bundle's `verifyPolicy` (the shipped CLI reads `DGK_TRUSTED_ISSUERS`, threaded through `buildWalletHost` → `createWalletCapabilities`; a wiring test asserts an untrusted issuer is rejected through the real host path, not only a test helper). The sovereign-citizen end-to-end proof (issue → policy verify → tamper reject → expired reject → strict revocation/issuer-trust → present → holder-binding → status-list revocation) passes. FOLLOW-UP (deferred by this ADR): the `trustRegistry` field is reserved — the static `trustedIssuers` list ships first; a dynamic registry is a future superset.
**Date**: 2026-07-01
