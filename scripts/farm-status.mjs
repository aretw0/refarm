 ...
 * Prefer `refarm runtime` for daily-driver runtime lifecycle.
...
function checkTractor() {
  ...
  if (!pid || !isAlive(pid)) {
    ...
    if (b42.bound && b42.pid !== farmhandPid) {
      ...
      info('tractor', `not running  ${c.dim}(start: refarm runtime ensure --wait)${c.reset}`);
    }
    return false;
  }
...
async function checkFarmhand() {
  ...
  if (!pid || !isAlive(pid)) {
    ...
    if (b01.bound) {
      ...
      info('farmhand', `not running  ${c.dim}(start: refarm runtime ensure --wait)${c.reset}`);
    }
    return false;
  }
