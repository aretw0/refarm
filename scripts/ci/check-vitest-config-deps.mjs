...
function checkDir(dir) {
	...
	for (const entry of readdirSync(dir, { withFileTypes: true })) {
		...
		const configPath = ["vitest.config.ts", "vitest.config.js"]
		...
		for (const imp of extractWorkspaceImports(source)) {
			if (!allDeclared.has(imp)) {
				errors.push({
					workspace:
						(dir === PACKAGES_DIR ? "packages" : "apps") + "/" + entry.name,
					missing: imp,
				});
			}
		}
...
if (allErrors.length === 0) {
	console.log(
		`${colors.green}✓ tool config phantom deps: none found${colors.reset}`,
	);
	process.exit(0);
}

console.error(
	`${colors.red}✗ tool config phantom dependencies detected${colors.reset}`,
);
console.error(
	`${colors.dim}  These work locally (pnpm hoisting) but fail in CI (.vite-temp context).${colors.reset}\n`,
);
for (const { workspace, missing } of allErrors) {
	console.error(
		`  ${colors.yellow}${workspace}${colors.reset}  missing devDependency: ${colors.red}${missing}${colors.reset}`,
	);
	console.error(
		`  ${colors.dim}  Fix: add "${missing}": "workspace:*" to ${workspace}/package.json devDependencies${colors.reset}`,
	);
}
console.error("");
process.exit(1);
