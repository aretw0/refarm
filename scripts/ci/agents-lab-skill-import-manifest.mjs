...
export function buildAgentsLabSkillImportManifest({ root = DEFAULT_ROOT } = {}) {
	...
	return {
		...
		mode: "plan-only",
		activationGate: {
			currentState: "ready-for-source-review",
			canStartNow: [
				...
				"Record accepted skill content, required edits, or upstream follow-up before installation.",
			],
			...
			stillBlockedBy: [
				"No reviewed source skill content in this repository yet.",
				"No Refarm skill invocation smoke selected for dogfood yet.",
		...
		boundaries: [
			...
			"Convention review is mandatory before copying or installing any skill.",
		...
		nextActions: [
			...
			"Only after review, install or vendor skills through the operator's Codex/Pi skill path.",
		],
