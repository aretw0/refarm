...
test("trusted publishing plan names the stage-only migration and exact GitHub claim", () => {
	...
	assert.equal(plan.selectionId, "ecosystem-ready");
	...
	// The inaugural packet is the proven union for vault-seed, rcdc5, coop-vault,
	// enem, arch-engine, and the personal/professional vaults.
	assert.equal(plan.packages.length, 34);
...
test("rcdc5-ready packages all carry the repository claim required by npm OIDC", () => {
	const plan = buildTrustedPublishingPlan({ selectionId: "rcdc5-ready" });

	assert.equal(plan.ok, true);
	assert.equal(plan.packages.length, 8);
	assert.deepEqual(plan.repositoryMismatches, []);
	for (const pkg of plan.packages) {
		assert.equal(pkg.repositoryUrl, plan.expectedRepositoryUrl, pkg.name);
	}
});
