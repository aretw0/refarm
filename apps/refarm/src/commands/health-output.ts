...
export function formatHealthSummary(report: HealthReport): string {
  ...
  if (report.results.alignment.length === 0) {
    ...
    report.results.alignment.forEach((issue: HealthIssue) => {
      lines.push(chalk.yellow(`   ⚠️  ${issue.package} main points to ${issue.entry} instead of dist/.`));
    });
  }
  ...
  if (!report.results.automations || report.results.automations.length === 0) {
    ...
    report.results.automations.forEach((issue: HealthIssue) => {
      lines.push(chalk.yellow(`   ⚠️  ${issue.file} ${issue.note ?? "has an invalid automation entry."}`));
    });
  }
  ...
  if (!report.results.namespaceWarnings || report.results.namespaceWarnings.length === 0) {
    lines.push(chalk.green("   ✅ Versioned root namespaces are declared or conventional infrastructure."));
  } else {
...
export function formatHealthPolicySummary(report: HealthPolicyReport): string {
  ...
  if (report.policy.ignoredGitVisibilityPatterns.length) {
    lines.push(`   Ignored git visibility patterns: ${report.policy.ignoredGitVisibilityPatterns.join(", ")}`);
  }
  if (report.policy.workspaceNamespaces?.length) {
    lines.push(`   Workspace namespaces: ${report.policy.workspaceNamespaces.map((namespace) => namespace.path).join(", ")}`);
  }
  if (report.policy.complexity?.enabled) {
    ...
    if (report.policy.complexity.allowedPatterns.length) {
      lines.push(`   Complexity allowed patterns: ${report.policy.complexity.allowedPatterns.join(", ")}`);
    }
  }
  return lines.join("\n");
}
...
export function formatHealthPolicySuggestionSummary(
  report: HealthPolicySuggestionReport,
): string {
  return [
