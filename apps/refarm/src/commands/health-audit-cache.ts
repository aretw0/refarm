...
function buildGitHealthAuditFingerprint(
	...
	try {
		...
		const status = readGitCommand(
			["status", "--porcelain=v1", "-z", "--untracked-files=all", "--", ...HEALTH_STATUS_PATHSPECS],
			{ cwd: root },
		);
		...
		appendGitChangedPathMetadata(
			hash,
			root,
			statusEntries.flatMap((entry) => entry.paths.filter(isHealthAuditRelevantStatusPath)),
		);
		return hash.digest("hex");
	} catch {
		return null;
...
function appendGitTrackedHealthFileMetadata(
	...
	for (const entry of entries) {
		appendHealthFingerprintValue(hash, "git:tracked-health-file", JSON.stringify(entry));
	}
}
...
function gitTrackedFileEntries(
	...
	return tracked
		...
		.filter((entry): entry is { mode: string; object: string; stage: string; path: string } =>
			Boolean(entry),
		);
}
...
function collectHealthFingerprintFiles(
	...
	const relativePath = normalizeHealthFingerprintPath(path.relative(rootDir, currentPath));
	if (stats.isDirectory()) {
		const directoryName = path.basename(currentPath);
		if (
			relativePath &&
			(directoryName.startsWith(".") || HEALTH_FINGERPRINT_SKIP_DIRS.has(directoryName))
		) {
			return;
		}
		let entries: string[];
