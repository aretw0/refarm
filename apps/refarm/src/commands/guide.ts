...
export function createGuideCommand(deps: GuideDeps = defaultGuideDeps()): Command {
	return new Command("guide")
		...
		.action(async (options: GuideOptions) => {
			...
			const infraTokens = (await silo.provision("object")) as unknown as Record<string, unknown>;
			...
			const modelStatus = modelCredentialStatus(modelProvider, modelTokens, deps.env());
			...
			const nextActions = checks.filter((check) => !check.ok).map((check) => check.action);
