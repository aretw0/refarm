...
function wantsJsonOutput(options: MigrateCommandOptions, command: Command): boolean {
	return options.json === true || command.optsWithGlobals<Record<string, unknown>>().json === true;
}
...
export const migrateCommand = new Command("migrate")
	...
	.action(async (options: MigrateCommandOptions, command: Command) => {
		try {
			await runMigrate(options, command);
		} catch (error) {
			// The target-URL prompt is the only interactive step here; a cancelled
...
async function runMigrate(options: MigrateCommandOptions, command: Command): Promise<void> {
	const json = wantsJsonOutput(options, command);
	if (!json) {
		console.log(chalk.red.bold("\nRepository mirror"));
		console.log(
			chalk.yellow("This process will mirror your entire repository to another Git remote.\n"),
		);
	}
	...
	if (configPath) {
		...
		if (!json) {
			console.warn(
				chalk.gray("No .refarm/config.json found in current directory. Using default context."),
			);
		}
		config = {
	...
	if (!config.brand?.urls?.repository) {
		...
		console.error(
			chalk.red(
				"Error: Could not detect source repository URL. Please ensure you are in a Git repo or have it in your config.",
			),
		);
		process.exitCode = 1;
		return;
