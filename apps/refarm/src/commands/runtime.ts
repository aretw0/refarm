...
import { RUNTIME_ENGINE_MODES } from "@refarm.dev/runtime";
...
`,
				...
				.action(async (opts: { wait?: boolean; json?: boolean }, subcommand: Command) => {
					...
					if (!stop.ok) {
						if (json) {
							printJson({
								...
								nextCommands: [RUNTIME_STOP_JSON_COMMAND, RUNTIME_DOCTOR_NEXT_COMMAND],
							});
						} else {
					...
					if (json) {
						const diagnostics =
							opts.wait && ready !== true ? runtimeStartDiagnostics(command) : undefined;
...
`,
				...
				.action(
					async (
						...
						if (!command) {
							if (opts.nextCommand) {
								const [nextCommand] = buildRuntimeJsonPayload(
									payload,
									{
										ensured: false,
										started: false,
					...
					async (
						...
						if (opts.dryRun) {
							if (json) {
								printJson(
									buildRuntimeJsonPayload(
										payload,
										{ launchCommand: command, dryRun: true },
										undefined,
										"start",
									),
								);
						...
						if (json) {
							printJson(
								buildRuntimeJsonPayload(
									payload,
									{ launchCommand: command, started: true },
									undefined,
									"start",
								),
							);
